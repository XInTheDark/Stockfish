...
Value Eval::evaluate(const Position& pos) {
  ...
  else
  {
      ...
      optimism += (int) optimism * (nnueComplexity + abs(simpleEval - nnue)) * (long long) 4029 / 2097152;
      nnue     -= (int) nnue     * (nnueComplexity + abs(simpleEval - nnue)) * (long long) 63 / 2097152;

      int npm = pos.non_pawn_material() * 31 / 2048;
      v = (  nnue     * (1011 + npm + 608 * pos.count<PAWN>() / 64)
           + optimism * (149 + npm +  61  * pos.count<PAWN>() / 64)) / 1024;
  }

  // Damp down the evaluation linearly when shuffling
  v = v * (215 - shuffling) / 214;
