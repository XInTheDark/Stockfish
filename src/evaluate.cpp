...
Value Eval::evaluate(const Position& pos) {
  ...
  else
  {
      ...
      optimism += (int) optimism * (nnueComplexity + abs(simpleEval - nnue)) * (long long) 2263 / 1048576;
      nnue -= (int) nnue * (nnueComplexity + abs(simpleEval - nnue)) * (long long) 65 / 2097152;

      int npm = pos.non_pawn_material() * 57 / 4096;
      v = (  nnue     * (997 + npm + 565 * pos.count<PAWN>() / 64)
           + optimism * (151 + npm + 57 * pos.count<PAWN>() / 64)) / 1024;
  }

  // Damp down the evaluation linearly when shuffling
  v = v * (216 - shuffling) / 214;
