...
Value Eval::evaluate(const Position& pos) {
  ...
  else
  {
      ...
      optimism += (int) optimism * (nnueComplexity + abs(simpleEval - nnue)) * (long long) 4427 / 2097152;
      nnue -= (int) nnue * (nnueComplexity + abs(simpleEval - nnue)) * (long long) 65 / 2097152;

      int npm = pos.non_pawn_material() * 15 / 1024;
      v = (  nnue     * (1012 + npm + 35 * pos.count<PAWN>() / 4)
           + optimism * (146 + npm +  15 * pos.count<PAWN>() / 16)) / 1024;
  }

  // Damp down the evaluation linearly when shuffling
  v = v * (214 - shuffling) / 214;
