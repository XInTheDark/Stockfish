...
void TimeManagement::init(Search::LimitsType& limits,
    ...
    if (limits.movestogo == 0)
    {
        ...
        if (originalTimeAdjust < 0)
            originalTimeAdjust = 0.3014 * std::log10(timeLeft) - 0.4877;

        // Calculate time constants based on current time left.
        double logTimeInSec = std::log10(scaledTime / 1000.0);
        double optConstant  = std::min(0.00307 + 0.000318 * logTimeInSec, 0.00499);
        double maxConstant  = std::max(3.48 + 2.94 * logTimeInSec, 2.94);

        optScale = std::min(0.0123 + std::pow(ply + 2.96, 0.464) * optConstant,
    ...
    maximumTime =
      TimePoint(std::min(0.824 * limits.time[us] - moveOverhead, maxScale * optimumTime)) - 10;
