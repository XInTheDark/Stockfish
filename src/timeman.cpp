...
void TimeManagement::init(Search::LimitsType& limits,
    ...
    if (limits.movestogo == 0)
    {
        ...
        if (originalTimeAdjust < 0)
            originalTimeAdjust = 0.3161 * std::log10(timeLeft) - 0.5036;

        // Calculate time constants based on current time left.
        double logTimeInSec = std::log10(scaledTime / 1000.0);
        double optConstant  = std::min(0.00301 + 0.000311 * logTimeInSec, 0.00497);
        double maxConstant  = std::max(3.34 + 3.01 * logTimeInSec, 2.89);

        optScale = std::min(0.0120 + std::pow(ply + 3.04, 0.455) * optConstant,
                            0.214 * limits.time[us] / timeLeft)
                 * originalTimeAdjust;

        maxScale = std::min(6.77, maxConstant + ply / 12.0);
    }
    ...
    maximumTime =
      TimePoint(std::min(0.822 * limits.time[us] - moveOverhead, maxScale * optimumTime)) - 10;
