...
Value futility_margin(Depth d, bool noTtCutNode, bool improving, bool oppWorsening) {
    Value futilityMult       = 107 - 30 * noTtCutNode;
...
Value to_corrected_static_eval(Value v, const Worker& w, const Position& pos, Stack* ss) {
    ...
    const auto cv =
      (6079 * pcv + 2982 * mcv + 3322 * macv + 6125 * micv + 6723 * (wnpcv + bnpcv) + cntcv * 5798)
      / 131072;
    v += cv;
...
int stat_bonus(Depth d) { return std::min(184 * d - 102, 1611); }

// History and stats update malus, based on depth
int stat_malus(Depth d) { return std::min(756 * d - 260, 2364); }
...
void Search::Worker::iterative_deepening() {
           ...
           && !(limits.depth && mainThread && rootDepth > limits.depth))
    {
        ...
        for (pvIdx = 0; pvIdx < multiPV; ++pvIdx)
        {
            ...
            delta     = 5 + std::abs(rootMoves[pvIdx].meanSquaredScore) / 13951;
            Value avg = rootMoves[pvIdx].averageScore;
            alpha     = std::max(avg - delta, -VALUE_INFINITE);
            beta      = std::min(avg + delta, VALUE_INFINITE);

            // Adjust optimism based on root move's averageScore (~4 Elo)
            optimism[us]  = 133 * avg / (std::abs(avg) + 85);
...
void Search::Worker::clear() {
    ...
    for (size_t i = 1; i < reductions.size(); ++i)
        reductions[i] = int((18.31 + std::log(size_t(options["Threads"])) / 2) * std::log(i));
...
Value Search::Worker::search(
  Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
    ...
    {
        int bonus = std::clamp(-10 * int((ss - 1)->staticEval + ss->staticEval), -1766, 1488) + 721;
    ...
    if (eval < alpha - 462 - 282 * depth * depth)
    {
    ...
    if (cutNode && (ss - 1)->currentMove != Move::null() && eval >= beta
        && ss->staticEval >= beta - 23 * depth + 414 && !excludedMove && pos.non_pawn_material(us)
        && ss->ply >= thisThread->nmpMinPly && beta > VALUE_TB_LOSS_IN_MAX_PLY)
    {
        assert(eval - beta >= 0);

        // Null move dynamic reduction based on depth and eval
        Depth R = std::min(int(eval - beta) / 223, 7) + depth / 3 + 5;
    ...
    probCutBeta = beta + 182 - 54 * improving - 30 * opponentWorsening;
    if (!PvNode && depth > 3
...
moves_loop:  // When in check, search starts here
    ...
    while ((move = mp.next_move(moveCountPruning)) != Move::none())
    {
        ...
        if (!rootNode && pos.non_pawn_material(us) && bestValue > VALUE_TB_LOSS_IN_MAX_PLY)
        {
            ...
            if (capture || givesCheck)
            {
                ...
                {
                    Value futilityValue = ss->staticEval + 302 + 252 * lmrDepth
                ...
                int seeHist = std::clamp(captHist / 33, -166 * depth, 161 * depth);
                if (!pos.see_ge(move, -170 * depth - seeHist))
                    continue;
            }
            else
            {
                ...
                if (history < -3893 * depth)
                    continue;

                history += 2 * thisThread->mainHistory[us][move.from_to()];

                lmrDepth += history / 3369;

                Value futilityValue =
                  ss->staticEval + (bestValue < ss->staticEval - 48 ? 138 : 47) + 144 * lmrDepth;
        ...
        if (ss->ply < thisThread->rootDepth * 2)
        {
                ...
                && ttData.depth >= depth - 3)
            {
                ...
                {
                    int doubleMargin = 259 * PvNode - 206 * !ttCapture;
                    int tripleMargin = 95 + 278 * PvNode - 262 * !ttCapture + 97 * ss->ttPv;
        ...
        ss->statScore = 2 * thisThread->mainHistory[us][move.from_to()]
                      + (*contHist[0])[movedPiece][move.to_sq()]
                      + (*contHist[1])[movedPiece][move.to_sq()] - 4164;

        // Decrease/increase reduction for moves with a good/bad history (~8 Elo)
        r -= ss->statScore / 11299;
        ...
        if (depth >= 2 && moveCount > 1)
        {
            ...
            if (value > alpha && d < newDepth)
            {
                ...
                const bool doDeeperSearch    = value > (bestValue + 39 + 2 * newDepth);  // (~1 Elo)
    ...
    {
        int bonus = (114 * (depth > 5) + 37 * !allNode + 166 * ((ss - 1)->moveCount > 7)
                     + 119 * (!ss->inCheck && bestValue <= ss->staticEval - 105)
                     + 126 * (!(ss - 1)->inCheck && bestValue <= -(ss - 1)->staticEval - 83));

        // Proportional to "how much damage we have to undo"
        bonus += std::min(-(ss - 1)->statScore / 105, 300);

        bonus = std::max(bonus, 0);

        update_continuation_histories(ss - 1, pos.piece_on(prevSq), prevSq,
                                      stat_bonus(depth) * bonus / 104);
        thisThread->mainHistory[~us][((ss - 1)->currentMove).from_to()]
          << stat_bonus(depth) * bonus / 175;
        ...
        && !(!bestMove && bestValue >= ss->staticEval))
    {
        ...
        thisThread->pawnCorrectionHistory[us][pawn_structure_index<Correction>(pos)]
          << bonus * 99 / 128;
        thisThread->materialCorrectionHistory[us][material_index(pos)] << bonus * 94 / 128;
...
Value Search::Worker::qsearch(Position& pos, Stack* ss, Value alpha, Value beta) {
    ...
    else
    {
        ...
        futilityBase = ss->staticEval + 292;
    }
...
Depth Search::Worker::reduction(bool i, Depth d, int mn, int delta) const {
    ...
    return (reductionScale + 1280 - delta * 798 / rootDelta) / 1024 + (!i && reductionScale > 1369);
}
