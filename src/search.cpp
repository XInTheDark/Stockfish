...
Value futility_margin(Depth d, bool noTtCutNode, bool improving, bool oppWorsening) {
    Value futilityMult       = 122 - 43 * noTtCutNode;
    Value improvingDeduction = 55 * improving * futilityMult / 32;
    Value worseningDeduction = (317 + 48 * improving) * oppWorsening * futilityMult / 1024;
...
Value to_corrected_static_eval(Value v, const Worker& w, const Position& pos) {
    ...
    v += cv * std::abs(cv) / 9287;
    return std::clamp(v, VALUE_TB_LOSS_IN_MAX_PLY + 1, VALUE_TB_WIN_IN_MAX_PLY - 1);
}

// History and stats update bonus, based on depth
int stat_bonus(Depth d) { return std::clamp(212 * d - 307, 16, 1280); }

// History and stats update malus, based on depth
int stat_malus(Depth d) { return (d < 4 ? 584 * d - 299 : 1369); }
...
void Search::Worker::iterative_deepening() {
           ...
           && !(limits.depth && mainThread && rootDepth > limits.depth))
    {
        ...
        for (pvIdx = 0; pvIdx < multiPV && !threads.stop; ++pvIdx)
        {
            ...
            delta     = 10 + avg * avg / 11048;
            alpha     = std::max(avg - delta, -VALUE_INFINITE);
            beta      = std::min(avg + delta, VALUE_INFINITE);

            // Adjust optimism based on root move's averageScore (~4 Elo)
            optimism[us]  = 130 * avg / (std::abs(avg) + 96);
...
Value Search::Worker::search(
  Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
    ...
    {
        int bonus = std::clamp(-14 * int((ss - 1)->staticEval + ss->staticEval), -1623, 1435);
    ...
    if (eval < alpha - 449 - (264 - 155 * ((ss + 1)->cutoffCnt > 3)) * depth * depth)
    {
    ...
    if (!PvNode && (ss - 1)->currentMove != Move::null() && (ss - 1)->statScore < 18055
        && eval >= beta && ss->staticEval >= beta - 21 * depth + 321 && !excludedMove
    ...
    probCutBeta = beta + 160 - 65 * improving;
    if (
      !PvNode && depth > 3
...
moves_loop:  // When in check, search starts here
    ...
    while ((move = mp.next_move(moveCountPruning)) != Move::none())
    {
        ...
        if (!rootNode && pos.non_pawn_material(us) && bestValue > VALUE_TB_LOSS_IN_MAX_PLY)
        {
            ...
            if (capture || givesCheck)
            {
                ...
                if (!givesCheck && lmrDepth < 7 && !ss->inCheck)
                {
                    ...
                    Value futilityValue =
                      ss->staticEval + 272 + 257 * lmrDepth + PieceValue[capturedPiece]
            ...
            else
            {
                ...
                if (lmrDepth < 6 && history < -4054 * depth)
                    continue;

                history += 63 * thisThread->mainHistory[us][move.from_to()] / 32;

                lmrDepth += history / 5106;

                Value futilityValue =
                  ss->staticEval + (bestValue < ss->staticEval - 53 ? 131 : 59) + 136 * lmrDepth;
        ...
        if (ss->ply < thisThread->rootDepth * 2)
        {
            ...
            if (!rootNode && move == ttMove && !excludedMove
                ...
                && tte->depth() >= depth - 3)
            {
                Value singularBeta  = ttValue - (43 + 39 * (ss->ttPv && !PvNode)) * depth / 64;
        ...
        ss->statScore = 2 * thisThread->mainHistory[us][move.from_to()]
                      + (*contHist[0])[movedPiece][move.to_sq()]
                      + (*contHist[1])[movedPiece][move.to_sq()]
                      + (*contHist[3])[movedPiece][move.to_sq()] - 4637;

        // Decrease/increase reduction for moves with a good/bad history (~8 Elo)
        r -= ss->statScore / 12964;
        ...
        if (value > bestValue)
        {
            ...
            if (value > alpha)
            {
                ...
                else
                {
                    ...
                    if (depth > 2 && depth < 12 && beta < 13931 && value > -13747)
                        depth -= 2;
    ...
    {
        int bonus = (depth > 5) + (PvNode || cutNode) + ((ss - 1)->statScore < -13852)
                  + ((ss - 1)->moveCount > 10)
                  + (!ss->inCheck && bestValue <= ss->staticEval - 139);
        update_continuation_histories(ss - 1, pos.piece_on(prevSq), prevSq,
...
Depth Search::Worker::reduction(bool i, Depth d, int mn, int delta) {
    ...
    return (reductionScale + 1227 - delta * 808 / rootDelta) / 1024 + (!i && reductionScale > 1000);
}
...
void update_all_stats(const Position& pos,
    ...
    {
        int bestMoveBonus = bestValue > beta + 183 ? quietMoveBonus      // larger bonus
