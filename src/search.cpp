...
Value futility_margin(Depth d, bool noTtCutNode, bool improving, bool oppWorsening) {
    Value futilityMult       = 130 - 40 * noTtCutNode;
    Value improvingDeduction = 54 * improving * futilityMult / 32;
    Value worseningDeduction = 376 * oppWorsening * futilityMult / 1024;
...
Value to_corrected_static_eval(Value v, const Worker& w, const Position& pos) {
    ...
    v += cv * std::abs(cv) / 5686;
    return std::clamp(v, VALUE_TB_LOSS_IN_MAX_PLY + 1, VALUE_TB_WIN_IN_MAX_PLY - 1);
}

// History and stats update bonus, based on depth
int stat_bonus(Depth d) { return std::clamp(201 * d - 300, 16, 1789); }

// History and stats update malus, based on depth
int stat_malus(Depth d) { return (d < 4 ? 846 * d - 287 : 1981); }
...
void Search::Worker::iterative_deepening() {
           ...
           && !(limits.depth && mainThread && rootDepth > limits.depth))
    {
        ...
        for (pvIdx = 0; pvIdx < multiPV && !threads.stop; ++pvIdx)
        {
            ...
            delta     = 9 + avg * avg / 10472;
            alpha     = std::max(avg - delta, -VALUE_INFINITE);
            beta      = std::min(avg + delta, VALUE_INFINITE);

            // Adjust optimism based on root move's averageScore (~4 Elo)
            optimism[us]  = 139 * avg / (std::abs(avg) + 98);
...
Value Search::Worker::search(
  Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
    ...
    {
        int bonus = std::clamp(-11 * int((ss - 1)->staticEval + ss->staticEval), -1610, 1300);
    ...
    // return a fail low.
    if (eval < alpha - 484 - 328 * depth * depth)
    {
    ...
    if (!PvNode && (ss - 1)->currentMove != Move::null() && (ss - 1)->statScore < 14947
        && eval >= beta && ss->staticEval >= beta - 23 * depth + 410 && !excludedMove
        ...
        && beta > VALUE_TB_LOSS_IN_MAX_PLY)
    {
        ...
        Depth R = std::min(int(eval - beta) / 190, 6) + depth / 3 + 5;
    ...
    probCutBeta = beta + 174 - 53 * improving;
    if (
      !PvNode && depth > 3
...
moves_loop:  // When in check, search starts here
    ...
    while ((move = mp.next_move(moveCountPruning)) != Move::none())
    {
        ...
        if (!rootNode && pos.non_pawn_material(us) && bestValue > VALUE_TB_LOSS_IN_MAX_PLY)
        {
            ...
            if (capture || givesCheck)
            {
                ...
                {
                    Value futilityValue = ss->staticEval + 238 + 257 * lmrDepth
                ...
                int seeHist = std::clamp(captHist * 21 / 512, -164 * depth, 155 * depth);
                if (!pos.see_ge(move, -187 * depth - seeHist))
                    continue;
            }
            else
            {
                ...
                if (lmrDepth < 6 && history < -4684 * depth)
                    continue;

                history += 2 * thisThread->mainHistory[us][move.from_to()];

                lmrDepth += history / 3535;

                Value futilityValue =
                  ss->staticEval + (bestValue < ss->staticEval - 50 ? 172 : 53) + 128 * lmrDepth;
        ...
        if (ss->ply < thisThread->rootDepth * 2)
        {
                ...
                && tte->depth() >= depth - 3)
            {
                ...
                {
                    int doubleMargin = 268 * PvNode - 213 * !ttCapture;
                    int tripleMargin = 110 + 300 * PvNode - 283 * !ttCapture + 106 * ss->ttPv;
        ...
        ss->statScore = 2 * thisThread->mainHistory[us][move.from_to()]
                      + (*contHist[0])[movedPiece][move.to_sq()]
                      + (*contHist[1])[movedPiece][move.to_sq()] - 5404;

        // Decrease/increase reduction for moves with a good/bad history (~8 Elo)
        r -= ss->statScore / 12045;
    ...
    {
        int bonus = (135 * (depth > 5) + 109 * (PvNode || cutNode)
                     + 198 * ((ss - 1)->statScore < -10940) + 126 * ((ss - 1)->moveCount > 11)
                     + 54 * (!ss->inCheck && bestValue <= ss->staticEval - 117)
                     + 145 * (!(ss - 1)->inCheck && bestValue <= -(ss - 1)->staticEval - 80));
        update_continuation_histories(ss - 1, pos.piece_on(prevSq), prevSq,
...
Value Search::Worker::qsearch(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth) {
    ...
    else
    {
        ...
        futilityBase = ss->staticEval + 257;
    }
...
Depth Search::Worker::reduction(bool i, Depth d, int mn, int delta) {
    ...
    return (reductionScale + 1414 - delta * 672 / rootDelta) / 1024 + (!i && reductionScale > 1094);
}
...
void update_all_stats(const Position& pos,
    ...
    {
        int bestMoveBonus = bestValue > beta + 154 ? quietMoveBonus      // larger bonus
