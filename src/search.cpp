...
//    a5 = 131072,
    ...
    a50 = 3, a51 = 4, a52 = 94, a53 = 6, a54 = 201, a55 = 58, a56 = 5, a57 = 400, a58 = 3,
//    a59 = 2,
//    a60 = 1024,
...
//    a111 = 8192,
    ...
    a120 = 8, a121 = 15, a122 = 2, a123 = 16,
//    a124 = 32,
    a125 = 800, a126 = 879, a127 = 220, a128 = 159, a129 = 94, a130 = 1501, a131 = 1080, a132 = 376, a133 = 2,
...
TUNE(a1,a2,a3,a4,a6,a7,a8,a9,a10,a11,a12,a13,a14,a15,a16,a17,a18,a19,a20,a21,a22,a23,a24,a25,a26,a27,a28,a29,a30,a31,a32,a33,a34,a35,a36,a37,a38,a39,a40,a41,a42,a43,a44,a45,a46,a47,a48,a49,a50,a51,a52,a53,a54,a55,a56,a57,a58,a61,a62,a63,a64,a65,a66,a67,a68,a69,a70,a72,a73,a74,a75,a76,a77,a78,a79,a80,a81,a82,a83,a84,a85,a86,a87,a88,a89,a90,a91,a92,a93,a94,a95,a96,a97,a98,a99,a100,a101,a102,a103,a104,a105,a106,a108,a109,a110,a112,a113,a114,a115,a116,a117,a118,a119,a120,a121,a122,a123,a125,a126,a127,a128,a129,a130,a131,a132,a133,a134,a135,a136,a137,a139,a140,a141,a142,a143,a144,a145,a146,a147,a148,a149,a150,a151,a152,a153,a154,a155,a156,a157,a158,a159,a160,a161,a162,a163);
TUNE(SetRange(-32, 32), a164);
TUNE(a165,a166,a167,a169,a170,a171,a172,a173,a174); // ext margins
...
int correction_value(const Worker& w, const Position& pos, const Stack* const ss) {
    ...
    return a1 * pcv + a2 * micv + a3 * (wnpcv + bnpcv) + a4 * cntcv;
}
...
void update_correction_history(const Position& pos,
    ...
    int nonPawnWeight = a6;

    workerThread.pawnCorrectionHistory[pawn_structure_index<Correction>(pos)][us]
      << bonus * a7 / 128;
    workerThread.minorPieceCorrectionHistory[minor_piece_index(pos)][us] << bonus * a8 / 128;
...
void Search::Worker::iterative_deepening() {
           ...
           && !(limits.depth && mainThread && rootDepth > limits.depth))
    {
        ...
        for (pvIdx = 0; pvIdx < multiPV; ++pvIdx)
        {
            ...
            delta     = a12 + std::abs(rootMoves[pvIdx].meanSquaredScore) / a13;
            Value avg = rootMoves[pvIdx].averageScore;
            alpha     = std::max(avg - delta, -VALUE_INFINITE);
            beta      = std::min(avg + delta, VALUE_INFINITE);

            // Adjust optimism based on root move's averageScore
            optimism[us]  = a14 * avg / (std::abs(avg) + a15);
...
void Search::Worker::clear() {
    mainHistory.fill(a16);
    lowPlyHistory.fill(a11);
    captureHistory.fill(-a17);
    pawnHistory.fill(-a18);
    pawnCorrectionHistory.fill(a19);
    ...
    for (size_t i = 1; i < reductions.size(); ++i)
        reductions[i] = int(a22 / 128.0 * std::log(i));
...
Value Search::Worker::search(
  Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
        ...
        if (ttData.move && ttData.value >= beta)
        {
            ...
            if (!ttCapture)
                update_quiet_histories(pos, ss, *this, ttData.move,
                                       std::min(a23 * depth - a24, a25));

            // Extra penalty for early quiet moves of the previous ply
            if (prevSq != SQ_NONE && (ss - 1)->moveCount <= 3 && !priorCapture)
                update_continuation_histories(ss - 1, pos.piece_on(prevSq), prevSq, -a26);
        }

        // Partial workaround for the graph history interaction problem
        // For high rule50 counts don't produce transposition table cutoffs.
        if (pos.rule50_count() < a27)
        {
    ...
    if (priorReduction >= a34 && !opponentWorsening)
        depth++;
    if (priorReduction >= a35 && depth >= a36 && ss->staticEval + (ss - 1)->staticEval > a37)
        depth--;

    // Step 7. Razoring
    // If eval is really low, skip search entirely and return the qsearch value.
    // For PvNodes, we must have a guard against mates being returned.
    if (!PvNode && eval < alpha - a38 - a39 * depth * depth)
    ...
    // The depth condition is important for mate finding.
    {
        auto futility_margin = [&](Depth d) {
            Value futilityMult = a40 - a41 * (cutNode && !ss->ttHit);

            return futilityMult * d                      //
                 - improving * futilityMult * 2          //
                 - opponentWorsening * futilityMult / 3  //
                 + (ss - 1)->statScore / a42             //
                 + std::abs(correctionValue) / a43;
        };

        if (!ss->ttPv && depth < a44 && eval - futility_margin(depth) >= beta && eval >= beta
    ...
    if (cutNode && (ss - 1)->currentMove != Move::null() && eval >= beta
        && ss->staticEval >= beta - a45 * depth + a46 && !excludedMove && pos.non_pawn_material(us)
        && ss->ply >= thisThread->nmpMinPly && !is_loss(beta))
    {
        assert(eval - beta >= 0);

        // Null move dynamic reduction based on depth and eval
        Depth R = std::min(int(eval - beta) / a47, a48) + depth / 3 + 5;
        ...
        {
            if (thisThread->nmpMinPly || depth < a49)
                return nullValue;
            ...
            thisThread->nmpMinPly = ss->ply + a50 * (depth - R) / a51;
    ...
    improving |= ss->staticEval >= beta + a52;

    // Step 10. Internal iterative reductions
    // For PV nodes without a ttMove as well as for deep enough cutNodes, we decrease depth.
    // (*Scaler) Especially if they make IIR less aggressive.
    if (!allNode && depth >= a53 && !ttData.move)
        depth--;

    // Step 11. ProbCut
    // If we have a good enough capture (or queen promotion) and a reduced search
    // returns a value much above beta, we can (almost) safely prune the previous move.
    probCutBeta = beta + a54 - a55 * improving;
    if (depth >= 3
        && !is_decisive(beta)
        ...
        && !(is_valid(ttData.value) && ttData.value < probCutBeta))
    {
        ...
        Depth      probCutDepth = std::max(depth - a56, 0);
...
moves_loop:  // When in check, search starts here
    ...
    while ((move = mp.next_move()) != Move::none())
    {
        ...
        if (!rootNode && pos.non_pawn_material(us) && !is_loss(bestValue))
        {
            ...
            if (moveCount >= (a58 + depth * depth) / (2 - improving))
                mp.skip_quiet_moves();
            ...
            if (capture || givesCheck)
            {
                ...
                if (!givesCheck && lmrDepth < a61 && !ss->inCheck)
                {
                    Value futilityValue = ss->staticEval + a62 + a63 * lmrDepth
                                        + PieceValue[capturedPiece] + a64 * captHist / 1024;
                    if (futilityValue <= alpha)
                        continue;
                }

                // SEE based pruning for captures and checks
                int seeHist = std::clamp(captHist / a65, -a66 * depth, a67 * depth);
                if (!pos.see_ge(move, -a68 * depth - seeHist))
                {
            ...
            else
            {
                ...
                Value baseFutility = (bestMove ? a73 : a74 + std::abs(history / a75));
                Value futilityValue =
                  ss->staticEval + baseFutility + a76 * lmrDepth + a77 * (ss->staticEval > alpha);

                // Futility pruning: parent node
                // (*Scaler): Generally, more frequent futility pruning
                // scales well with respect to time and threads
                if (!ss->inCheck && lmrDepth < a78 && futilityValue <= alpha)
                {
                ...
                if (!pos.see_ge(move, -a79 * lmrDepth * lmrDepth))
                    continue;
            }
        }
        ...
        if (!rootNode && move == ttData.move && !excludedMove
            && depth >= a80 - (thisThread->completedDepth > a81) + ss->ttPv && is_valid(ttData.value)
            && !is_decisive(ttData.value) && (ttData.bound & BOUND_LOWER)
            && ttData.depth >= depth - 3)
        {
            Value singularBeta  = ttData.value - (a82 + a83 * (ss->ttPv && !PvNode)) * depth / a84;
        ...
        if (ss->ttPv)
            r -= a89 + PvNode * a90 + (ttData.value > alpha) * a91
               + (ttData.depth >= depth) * (943 + cutNode * 1180);

        // These reduction adjustments have no proven non-linear scaling

        r += a92;  // Base reduction offset to compensate for other tweaks
        r -= moveCount * a93;
        r -= std::abs(correctionValue) / a94;

        // Increase reduction for cut nodes
        if (cutNode)
            r += a95 + a96 * !ttData.move;
        ...
        if ((ss + 1)->cutoffCnt > a100)
            r += a101 + allNode * a102;

        if (!capture && !givesCheck && ss->quietMoveStreak >= a103)
            r += (ss->quietMoveStreak - 1) * a104;
        ...
        if (depth >= 2 && moveCount > 1)
        {
            ...
            if (value > alpha && d < newDepth)
            {
                ...
                const bool doDeeperSearch    = value > (bestValue + a112 + a113 * newDepth);
                const bool doShallowerSearch = value < bestValue + a114;
                ...
                update_continuation_histories(ss, movedPiece, move.to_sq(), a115);
            }
            else if (value > alpha && value < bestValue + a114)
                newDepth--;
        }
        ...
        else if (!PvNode || moveCount > 1)
        {
            ...
            r -= ttMoveHistory / a117;

            // Note that if expected reduction is high, we reduce search depth here
            value = -search<NonPV>(pos, ss + 1, -(alpha + 1), -alpha,
                                   newDepth - (r > a118) - (r > a119 && newDepth > 2), !cutNode);
        }
        ...
        if (PvNode && (moveCount == 1 || value > alpha))
        {
            ...
            if (move == ttData.move && thisThread->rootDepth > a120)
        ...
        int inc = (value == bestValue && ss->ply + 2 >= thisThread->rootDepth
                   && (int(nodes) & a121) == 0 && !is_win(std::abs(value) + 1));
        ...
        if (value + inc > bestValue)
        {
            ...
            if (value + inc > alpha)
            {
                ...
                if (value >= beta)
                {
                    ...
                    ss->cutoffCnt += (extension < a122) || PvNode;
                    assert(value >= beta);  // Fail high
                    break;
                }

                // Reduce other moves if we have found at least one score improvement
                if (depth > 2 && depth < a123 && !is_decisive(value))
                    depth -= 2;
    ...
    else if (bestMove)
    {
        ...
        if (!PvNode)
            ttMoveHistory << (bestMove == ttData.move ? a125 : -a126);
    }
    ...
    {
        int bonusScale = -a127;
        bonusScale += std::min(-(ss - 1)->statScore / a175, a176);
        bonusScale += std::min(a177 * depth, a178);
        bonusScale += a179 * ((ss - 1)->moveCount > a180);
        bonusScale += a181 * (!ss->inCheck && bestValue <= ss->staticEval - a182);
        bonusScale += a183 * (!(ss - 1)->inCheck && bestValue <= -(ss - 1)->staticEval - a184);
    ...
    else if (priorCapture && prevSq != SQ_NONE)
    {
        ...
        thisThread->captureHistory[pos.piece_on(prevSq)][prevSq][type_of(capturedPiece)] << a131;
    }
...
Value Search::Worker::qsearch(Position& pos, Stack* ss, Value alpha, Value beta) {
    ...
    else
    {
        ...
        futilityBase = ss->staticEval + a132;
    }
...
void update_all_stats(const Position&      pos,
    ...
    int bonus = std::min(a140 * depth - a141, a142) + a143 * (bestMove == ttMove);
    int malus = std::min(a144 * depth - a145, a146) - a147 * moveCount;

    if (!pos.capture_stage(bestMove))
    {
        update_quiet_histories(pos, ss, workerThread, bestMove, bonus * a148 / 1024);

        // Decrease stats for all non-best quiet moves
        for (Move move : quietsSearched)
            update_quiet_histories(pos, ss, workerThread, move, -malus * a149 / 1024);
    }
    else
    {
        // Increase stats for the best move in case it was a capture move
        capturedPiece = type_of(pos.piece_on(bestMove.to_sq()));
        captureHistory[movedPiece][bestMove.to_sq()][capturedPiece] << bonus * a150 / 1024;
    }

    // Extra penalty for a quiet early move that was not a TT move in
    // previous ply when it gets refuted.
    if (prevSq != SQ_NONE && ((ss - 1)->moveCount == 1 + (ss - 1)->ttHit) && !pos.captured_piece())
        update_continuation_histories(ss - 1, pos.piece_on(prevSq), prevSq, -malus * a151 / 1024);
    ...
    for (Move move : capturesSearched)
    {
        ...
        captureHistory[movedPiece][move.to_sq()][capturedPiece] << -malus * a152 / 1024;
    }
}
...
void update_continuation_histories(Stack* ss, Piece pc, Square to, int bonus) {
    std::array<ConthistBonus, 6> conthist_bonuses = {
      {{1, a153}, {2, a154}, {3, a155}, {4, a156}, {5, a157}, {6, a158}}};
...
void update_quiet_histories(
  const Position& pos, Stack* ss, Search::Worker& workerThread, Move move, int bonus) {
    ...
    if (ss->ply < LOW_PLY_HISTORY_SIZE)
        workerThread.lowPlyHistory[ss->ply][move.from_to()] << bonus * a159 / 1024;

    update_continuation_histories(ss, pos.moved_piece(move), move.to_sq(),
                                  bonus * (bonus > 0 ? a160 : a161) / 1024);

    int pIndex = pawn_structure_index(pos);
    workerThread.pawnHistory[pIndex][pos.moved_piece(move)][move.to_sq()]
      << bonus * (bonus > 0 ? a162 : a163) / 1024;
}
