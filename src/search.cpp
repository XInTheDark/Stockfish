...
Value futility_margin(Depth d, bool noTtCutNode, bool improving) {
    return Value((123 - 44 * noTtCutNode) * (d - improving));
}
...
Depth reduction(bool i, Depth d, int mn, Value delta, Value rootDelta) {
    ...
    return (reductionScale + 1499 - int(delta) * 961 / int(rootDelta)) / 1024
         + (!i && reductionScale > 965);
}
...
int stat_bonus(Depth d) { return std::min(326 * d - 418, 1750); }

// History and stats update malus, based on depth
int stat_malus(Depth d) { return std::min(471 * d - 480, 1419); }
...
void Search::init() {
    ...
    for (int i = 1; i < MAX_MOVES; ++i)
        Reductions[i] = int((19.05 + std::log(Threads.size()) / 2) * std::log(i));
}
...
void Thread::search() {
           ...
           && !(Limits.depth && mainThread && rootDepth > Limits.depth))
    {
        ...
        for (pvIdx = 0; pvIdx < multiPV && !Threads.stop; ++pvIdx)
        {
            ...
            delta     = Value(10) + int(avg) * avg / 15308;
            alpha     = std::max(avg - delta, -VALUE_INFINITE);
            beta      = std::min(avg + delta, VALUE_INFINITE);

            // Adjust optimism based on root move's averageScore (~4 Elo)
            optimism[us]  = 110 * (avg + 33) / (std::abs(avg + 32) + 119);
            optimism[~us] = -115 * (avg + 41) / (std::abs(avg + 14) + 123);
...
Value search(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
    ...
    {
        int bonus = std::clamp(-16 * int((ss - 1)->staticEval + ss->staticEval), -2023, 2023);
    ...
    if (eval < alpha - 520 - (269 - 175 * ((ss + 1)->cutoffCnt > 3)) * depth * depth)
    {
    ...
    if (!ss->ttPv && depth < 11
        && eval - futility_margin(depth, cutNode && !ss->ttHit, improving)
               - (ss - 1)->statScore / 323
             >= beta
        && eval >= beta && eval < 27978  // smaller than TB wins
        && (!ttMove || ttCapture))
        return eval;

    // Step 9. Null move search with verification search (~35 Elo)
    if (!PvNode && (ss - 1)->currentMove != MOVE_NULL && (ss - 1)->statScore < 19568 && eval >= beta
        && eval >= ss->staticEval && ss->staticEval >= beta - 23 * depth + 291 && !excludedMove
    ...
    // if current depth is not too low.
    if (cutNode && depth >= 7 && !ttMove)
        depth -= 2;

    probCutBeta = beta + 175 - 73 * improving;
...
moves_loop:  // When in check, search starts here
    ...
    while ((move = mp.next_move(moveCountPruning)) != MOVE_NONE)
    {
        ...
        if (!rootNode && pos.non_pawn_material(us) && bestValue > VALUE_TB_LOSS_IN_MAX_PLY)
        {
            ...
            if (capture || givesCheck)
            {
                ...
                if (!givesCheck && lmrDepth < 6 && !ss->inCheck)
                {
                    ...
                    int   futilityEval =
                      ss->staticEval + 228 + 253 * lmrDepth + PieceValue[capturedPiece]
                ...
                if (!pos.see_ge(move, Value(-181) * depth))
                    continue;
            }
            else
            {
                ...
                if (lmrDepth < 6 && history < -3522 * depth)
                    continue;

                history += 2 * thisThread->mainHistory[us][from_to(move)];

                lmrDepth += history / 8394;
                lmrDepth = std::max(lmrDepth, -1);

                // Futility pruning: parent node (~13 Elo)
                if (!ss->inCheck && lmrDepth < 13
                    && ss->staticEval + (bestValue < ss->staticEval - 55 ? 142 : 70)
                           + 112 * lmrDepth
                         <= alpha)
                    continue;
        ...
        if (ss->ply < thisThread->rootDepth * 2)
        {
            ...
            if (!rootNode && move == ttMove && !excludedMove
                ...
                && tte->depth() >= depth - 3)
            {
                Value singularBeta  = ttValue - (60 + 55 * (ss->ttPv && !PvNode)) * depth / 64;
                ...
                if (value < singularBeta)
                {
                    ...
                    if (!PvNode && value < singularBeta - 16 && ss->doubleExtensions <= 11)
                    {
                        extension = 2;
                        depth += depth < 15;
                    }
            ...
            // Quiet ttMove extensions (~1 Elo)
                     ...
                     && (*contHist[0])[movedPiece][to_sq(move)] >= 4760)
                extension = 1;
        }
        ...
        ss->statScore = 2 * thisThread->mainHistory[us][from_to(move)]
                      ...
                      + (*contHist[1])[movedPiece][to_sq(move)]
                      + (*contHist[3])[movedPiece][to_sq(move)] - 4009;

        // Decrease/increase reduction for moves with a good/bad history (~25 Elo)
        r -= ss->statScore / (10143 + 4367 * (depth > 5 && depth < 29));
            ...
            && (!ss->ttPv || !capture || (cutNode && (ss - 1)->moveCount > 1)))
        {
            ...
            if (value > alpha && d < newDepth)
            {
                ...
                const bool doDeeperSearch     = value > (bestValue + 56 + 10 * (newDepth - d));
                const bool doEvenDeeperSearch = value > alpha + 635 && ss->doubleExtensions <= 6;
        ...
        if (value > bestValue)
        {
            ...
            if (value > alpha)
            {
                ...
                else
                {
                    ...
                    if (depth > 2 && depth < 10 && beta < 14519 && value > -11555)
                        depth -= 2;
    ...
    {
        int bonus = (depth > 6) + (PvNode || cutNode) + (bestValue < alpha - 702)
                  + ((ss - 1)->moveCount > 11);
        update_continuation_histories(ss - 1, pos.piece_on(prevSq), prevSq,
...
void update_all_stats(const Position& pos,
    ...
    if (!pos.capture_stage(bestMove))
    {
        ...
        int moveMalus = bestValue > beta + 177 ? quietMoveMalus      // larger malus
