...
namespace {
  ...
  Value futility_margin(Depth d, bool noTtCutNode, bool improving) {
    return Value((133 - 42 * noTtCutNode) * (d - improving));
  }
  ...
  Depth reduction(bool i, Depth d, int mn, Value delta, Value rootDelta) {
    ...
    return  (reductionScale + 1444 - int(delta) * 977 / int(rootDelta)) / 1024
          + (!i && reductionScale > 839);
  }
...
void Search::init() {
  ...
  for (int i = 1; i < MAX_MOVES; ++i)
      Reductions[i] = int((20.62 + std::log(Threads.size()) / 2) * std::log(i));
}
...
namespace {
  ...
  Value search(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
    ...
    {
        int bonus = std::clamp(-19 * int((ss-1)->staticEval + ss->staticEval), -1876, 1876);
    ...
    // return a fail low.
    if (eval < alpha - 548 - 247 * depth * depth)
    {
    ...
    if (   !ss->ttPv
        ...
        &&  eval >= beta
        &&  eval < 26265) // smaller than TB wins
        return eval;

    // Step 9. Null move search with verification search (~35 Elo)
    if (   !PvNode
        && (ss-1)->currentMove != MOVE_NULL
        && (ss-1)->statScore < 17379
        &&  eval >= beta
        &&  eval >= ss->staticEval
        &&  ss->staticEval >= beta - 22 * depth + 267
        && !excludedMove
        ...
        &&  beta > VALUE_TB_LOSS_IN_MAX_PLY)
    {
        ...
        Depth R = std::min(int(eval - beta) / 146, 6) + depth / 3 + 5;
    ...
    probCutBeta = beta + 165 - 70 * improving;
...
moves_loop: // When in check, search starts here
    ...
    while ((move = mp.next_move(moveCountPruning)) != MOVE_NONE)
    {
          ...
          && bestValue > VALUE_TB_LOSS_IN_MAX_PLY)
      {
              ...
              || givesCheck)
          {
              ...
              if (   !givesCheck
                  && lmrDepth < 7
                  && !ss->inCheck
                  && ss->staticEval + 194 + 200 * lmrDepth + PieceValue[pos.piece_on(to_sq(move))]
          ...
          else
          {
              ...
              lmrDepth += history / 6735;
              lmrDepth = std::max(lmrDepth, -2);

              // Futility pruning: parent node (~13 Elo)
              if (   !ss->inCheck
                  && lmrDepth < 14
                  && ss->staticEval + 116 + 143 * lmrDepth <= alpha)
                  continue;
      ...
      if (ss->ply < thisThread->rootDepth * 2)
      {
          ...
          if (   !rootNode
           ...
           /* &&  ttValue != VALUE_NONE Already implicit in the next condition */
              ...
              &&  tte->depth() >= depth - 3)
          {
              Value singularBeta = ttValue - (67 + 59 * (ss->ttPv && !PvNode)) * depth / 64;
          ...
          // Quiet ttMove extensions (~1 Elo)
                   ...
                   && move == ttMove
                   && move == ss->killers[0]
                   && (*contHist[0])[movedPiece][to_sq(move)] >= 4555)
              extension = 1;
      }
      ...
      r -= ss->statScore / (10654 + 4287 * (depth > 5 && depth < 23));
              ...
              || (cutNode && (ss-1)->moveCount > 1)))
      {
          ...
          if (value > alpha && d < newDepth)
          {
              ...
              const bool doDeeperSearch = value > (bestValue + 61 + 10 * (newDepth - d));
              const bool doEvenDeeperSearch = value > alpha + 747 && ss->doubleExtensions <= 6;
    ...
    */
    ...
    {
        int bonus = (depth > 6) + (PvNode || cutNode) + (bestValue < alpha - 723) + ((ss-1)->moveCount > 11);
  ...
  Value qsearch(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth) {
    ...
    else
    {
        ...
        futilityBase = std::min(ss->staticEval, bestValue) + 203;
    }
