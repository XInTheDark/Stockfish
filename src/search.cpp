  ...
  Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
    ...
    if (!rootNode)
    {
        ...
        if (threads.stop.load(std::memory_order_relaxed) || pos.is_draw(ss->ply)
            || ss->ply >= MAX_PLY)
            return (ss->ply >= MAX_PLY && !ss->inCheck) ? evaluate(pos, optimism[us], rootPos)
    ...
    else if (ss->ttHit)
    {
        ...
        if (eval == VALUE_NONE)
            unadjustedStaticEval = ss->staticEval = eval = evaluate(pos, optimism[us], rootPos);
        else if (PvNode)
            Eval::NNUE::hint_common_parent_position(pos);
...
Value Search::Worker::qsearch(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth) {
    ...
    else
    {
        if (ss->ttHit)
        {
            ...
            if ((unadjustedStaticEval = ss->staticEval = bestValue = tte->eval()) == VALUE_NONE)
                unadjustedStaticEval = ss->staticEval = bestValue =
                  evaluate(pos, optimism[us], rootPos);
        ...
        else
        {
            ...
            unadjustedStaticEval = ss->staticEval = bestValue =
              (ss - 1)->currentMove != Move::null() ? evaluate(pos, optimism[us], rootPos)
                                                    : -(ss - 1)->staticEval;
