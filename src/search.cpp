...
Value futility_margin(Depth d, bool noTtCutNode, bool improving, bool oppWorsening) {
    Value futilityMult       = 125 - 41 * noTtCutNode;
    Value improvingDeduction = 54 * improving * futilityMult / 32;
    Value worseningDeduction = (311 + 50 * improving) * oppWorsening * futilityMult / 1024;
...
Value to_corrected_static_eval(Value v, const Worker& w, const Position& pos) {
    ...
    v += cv * std::abs(cv) / 8751;
    return std::clamp(v, VALUE_TB_LOSS_IN_MAX_PLY + 1, VALUE_TB_WIN_IN_MAX_PLY - 1);
}

// History and stats update bonus, based on depth
int stat_bonus(Depth d) { return std::clamp(209 * d - 304, 16, 1315); }

// History and stats update malus, based on depth
int stat_malus(Depth d) { return (d < 4 ? 608 * d - 294 : 1282); }
...
void Search::Worker::iterative_deepening() {
           ...
           && !(limits.depth && mainThread && rootDepth > limits.depth))
    {
        ...
        for (pvIdx = 0; pvIdx < multiPV && !threads.stop; ++pvIdx)
        {
            ...
            delta     = 10 + avg * avg / 10602;
            alpha     = std::max(avg - delta, -VALUE_INFINITE);
            beta      = std::min(avg + delta, VALUE_INFINITE);

            // Adjust optimism based on root move's averageScore (~4 Elo)
            optimism[us]  = 127 * avg / (std::abs(avg) + 95);
...
void Search::Worker::clear() {
    ...
    for (size_t i = 1; i < reductions.size(); ++i)
        reductions[i] = int((18.69 + std::log(size_t(options["Threads"])) / 2) * std::log(i));
...
Value Search::Worker::search(
  Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
    ...
    {
        int bonus = std::clamp(-14 * int((ss - 1)->staticEval + ss->staticEval), -1630, 1481);
    ...
    if (eval < alpha - 466 - (260 - 158 * ((ss + 1)->cutoffCnt > 3)) * depth * depth)
    {
    ...
    if (!PvNode && (ss - 1)->currentMove != Move::null() && (ss - 1)->statScore < 18383
        && eval >= beta && ss->staticEval >= beta - 21 * depth + 325 && !excludedMove
    ...
    probCutBeta = beta + 163 - 64 * improving;
    if (
      !PvNode && depth > 3
...
moves_loop:  // When in check, search starts here
    ...
    while ((move = mp.next_move(moveCountPruning)) != Move::none())
    {
        ...
        if (!rootNode && pos.non_pawn_material(us) && bestValue > VALUE_TB_LOSS_IN_MAX_PLY)
        {
            ...
            if (capture || givesCheck)
            {
                ...
                {
                    Value futilityValue = ss->staticEval + 272 + 261 * lmrDepth
                ...
                int seeHist = std::clamp(captHist / 32, -194 * depth, 194 * depth);
                if (!pos.see_ge(move, -196 * depth - seeHist))
                    continue;
            }
            else
            {
                ...
                if (lmrDepth < 6 && history < -4296 * depth)
                    continue;

                history += 33 * thisThread->mainHistory[us][move.from_to()] / 16;

                lmrDepth += history / 4968;

                Value futilityValue =
                  ss->staticEval + (bestValue < ss->staticEval - 55 ? 129 : 60) + 144 * lmrDepth;
                ...
                if (!pos.see_ge(move, -29 * lmrDepth * lmrDepth))
                    continue;
            }
        }
        ...
        if (ss->ply < thisThread->rootDepth * 2)
        {
            ...
            if (!rootNode && move == ttMove && !excludedMove
                ...
                && tte->depth() >= depth - 3)
            {
                Value singularBeta  = ttValue - (43 + 40 * (ss->ttPv && !PvNode)) * depth / 64;
        ...
        ss->statScore = 2 * thisThread->mainHistory[us][move.from_to()]
                      + (*contHist[0])[movedPiece][move.to_sq()]
                      + (*contHist[1])[movedPiece][move.to_sq()]
                      + (*contHist[3])[movedPiece][move.to_sq()] - 4721;

        // Decrease/increase reduction for moves with a good/bad history (~8 Elo)
        r -= ss->statScore / 13543;
        ...
        if (value > bestValue)
        {
            ...
            if (value > alpha)
            {
                ...
                else
                {
                    ...
                    if (depth > 2 && depth < 11 && beta < 13964 && value > -13589)
                        depth -= 2;
    ...
    {
        int bonus = (depth > 5) + (PvNode || cutNode) + ((ss - 1)->statScore < -13727)
                  + ((ss - 1)->moveCount > 10)
...
Depth Search::Worker::reduction(bool i, Depth d, int mn, int delta) {
    ...
    return (reductionScale + 1230 - delta * 811 / rootDelta) / 1024 + (!i && reductionScale > 1043);
}
...
void update_all_stats(const Position& pos,
    ...
    {
        int bestMoveBonus = bestValue > beta + 193 ? quietMoveBonus      // larger bonus
