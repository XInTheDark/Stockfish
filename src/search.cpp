...
void Search::Worker::iterative_deepening() {
           ...
           && !(limits.depth && mainThread && rootDepth > limits.depth))
    {
        ...
        if (limits.use_time_management() && !threads.stop && !mainThread->stopOnPonderhit)
        {
            ...
            double fallingEval = (1079 + 224 * (mainThread->bestPreviousAverageScore - bestValue)
                                  + 97 * (mainThread->iterValue[iterIdx] - bestValue))
                               / 10000.0;
            fallingEval = std::clamp(fallingEval, 0.582, 1.706);

            // If the bestMove is stable over several iterations, reduce time accordingly
            timeReduction    = lastBestMoveDepth + 8 < completedDepth ? 1.531 : 0.711;
            double reduction = (1.46 + mainThread->previousTimeReduction) / (2.24 * timeReduction);
            double bestMoveInstability = 1 + 1.89 * totBestMoveChanges / threads.size();
            double recapture           = limits.capSq == rootMoves[0].pv[0].to_sq() ? 0.934 : 0.975;
            ...
            if (completedDepth >= 10 && nodesEffort >= 95 && elapsedTime > totalTime * 0.754
                && !mainThread->ponder)
                threads.stop = true;
