...
moves_loop: // When in check, search starts here
    ...
    while ((move = mp.next_move(moveCountPruning)) != MOVE_NONE)
    {
          ...
          && bestValue > VALUE_TB_LOSS_IN_MAX_PLY)
      {
              ...
              || givesCheck)
          {
              ...
              if (   !givesCheck
                  && lmrDepth < 6
                  && !ss->inCheck
                  && ss->staticEval + 183 + 256 * lmrDepth + PieceValue[EG][pos.piece_on(to_sq(move))]
                   + captureHistory[movedPiece][to_sq(move)][type_of(pos.piece_on(to_sq(move)))] * 1094 / 7000 < alpha)
                  continue;
          ...
          else
          {
              ...
              if (   !ss->inCheck
                  && lmrDepth < 13
                  && ss->staticEval + 99 + 148 * lmrDepth <= alpha)
                  continue;

              lmrDepth = std::max(lmrDepth, 0);

              // Prune moves with negative SEE (~4 Elo)
              if (!pos.see_ge(move, Value(-21 * lmrDepth * lmrDepth - 13 * lmrDepth)))
                  continue;
          }
      }
      ...
      if (ss->ply < thisThread->rootDepth * 2)
      {
          ...
          if (   !rootNode
           ...
           /* &&  ttValue != VALUE_NONE Already implicit in the next condition */
              ...
              &&  tte->depth() >= depth - 3)
          {
              Value singularBeta = ttValue - (4 + 2 * (ss->ttPv && !PvNode)) * depth / 2;
          ...
          // Quiet ttMove extensions (~1 Elo)
                   ...
                   && move == ttMove
                   && move == ss->killers[0]
                   && (*contHist[0])[movedPiece][to_sq(move)] >= 6172)
              extension = 1;
      }
      ...
      r -= ss->statScore / (10100 + 5237 * (depth > 7 && depth < 22));
              ...
              || (cutNode && (ss-1)->moveCount > 1)))
      {
          ...
          if (value > alpha && d < newDepth)
          {
              ...
              const bool doDeeperSearch = value > (alpha + 51 + 13 * (newDepth - d));
              const bool doEvenDeeperSearch = value > alpha + 705 && ss->doubleExtensions <= 5;
