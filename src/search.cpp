...
Value futility_margin(Depth d, bool noTtCutNode, bool improving, bool oppWorsening) {
    Value futilityMult       = 128 - 43 * noTtCutNode;
    Value improvingDeduction = 58 * improving * futilityMult / 32;
    Value worseningDeduction = 342 * oppWorsening * futilityMult / 1024;
...
Value to_corrected_static_eval(Value v, const Worker& w, const Position& pos) {
    ...
    v += cv * std::abs(cv) / 5333;
    return std::clamp(v, VALUE_TB_LOSS_IN_MAX_PLY + 1, VALUE_TB_WIN_IN_MAX_PLY - 1);
}

// History and stats update bonus, based on depth
int stat_bonus(Depth d) { return std::clamp(192 * d - 284, 20, 1455); }

// History and stats update malus, based on depth
int stat_malus(Depth d) { return (d < 4 ? 732 * d - 265 : 2080); }
...
void Search::Worker::iterative_deepening() {
           ...
           && !(limits.depth && mainThread && rootDepth > limits.depth))
    {
        ...
        for (pvIdx = 0; pvIdx < multiPV && !threads.stop; ++pvIdx)
        {
            ...
            delta     = 9 + avg * avg / 10678;
            alpha     = std::max(avg - delta, -VALUE_INFINITE);
            beta      = std::min(avg + delta, VALUE_INFINITE);

            // Adjust optimism based on root move's averageScore (~4 Elo)
            optimism[us]  = 126 * avg / (std::abs(avg) + 88);
...
void Search::Worker::clear() {
    ...
    for (size_t i = 1; i < reductions.size(); ++i)
        reductions[i] = int((19.78 + std::log(size_t(options["Threads"])) / 2) * std::log(i));
...
Value Search::Worker::search(
  Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
    ...
    {
        int bonus = std::clamp(-10 * int((ss - 1)->staticEval + ss->staticEval), -1499, 1324);
    ...
    // return a fail low.
    if (eval < alpha - 513 - 293 * depth * depth)
    {
    ...
    if (!PvNode && (ss - 1)->currentMove != Move::null() && (ss - 1)->statScore < 13897
        && eval >= beta && ss->staticEval >= beta - 21 * depth + 391 && !excludedMove
    ...
    probCutBeta = beta + 172 - 59 * improving;
    if (
      !PvNode && depth > 3
...
moves_loop:  // When in check, search starts here
    ...
    while ((move = mp.next_move(moveCountPruning)) != Move::none())
    {
        ...
        if (!rootNode && pos.non_pawn_material(us) && bestValue > VALUE_TB_LOSS_IN_MAX_PLY)
        {
            ...
            if (capture || givesCheck)
            {
                ...
                {
                    Value futilityValue = ss->staticEval + 293 + 236 * lmrDepth
                ...
                int seeHist = std::clamp(captHist / 32, -177 * depth, 165 * depth);
            ...
            else
            {
                ...
                lmrDepth += history / 3829;

                Value futilityValue =
                  ss->staticEval + (bestValue < ss->staticEval - 52 ? 138 : 53) + 144 * lmrDepth;
        ...
        if (ss->ply < thisThread->rootDepth * 2)
        {
            ...
            if (!rootNode && move == ttMove && !excludedMove
                ...
                && tte->depth() >= depth - 3)
            {
                Value singularBeta  = ttValue - (55 + 79 * (ss->ttPv && !PvNode)) * depth / 64;
                ...
                {
                    int doubleMargin = 294 * PvNode - 190 * !ttCapture;
                    int tripleMargin = 108 + 253 * PvNode - 289 * !ttCapture + 100 * ss->ttPv;
        ...
        ss->statScore = 2 * thisThread->mainHistory[us][move.from_to()]
                      + (*contHist[0])[movedPiece][move.to_sq()]
                      + (*contHist[1])[movedPiece][move.to_sq()] - 4787;

        // Decrease/increase reduction for moves with a good/bad history (~8 Elo)
        r -= ss->statScore / 11143;
    ...
    {
        int bonus = (109 * (depth > 5) + 116 * (PvNode || cutNode)
                     + 196 * ((ss - 1)->statScore < -14589) + 119 * ((ss - 1)->moveCount > 8)
                     + 63 * (!ss->inCheck && bestValue <= ss->staticEval - 109)
                     + 145 * (!(ss - 1)->inCheck && bestValue <= -(ss - 1)->staticEval - 76));
        update_continuation_histories(ss - 1, pos.piece_on(prevSq), prevSq,
...
Value Search::Worker::qsearch(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth) {
    ...
    else
    {
        ...
        futilityBase = ss->staticEval + 290;
    }
...
Depth Search::Worker::reduction(bool i, Depth d, int mn, int delta) {
    ...
    return (reductionScale + 1246 - delta * 744 / rootDelta) / 1024 + (!i && reductionScale > 1318);
}
