...
namespace {
  ...
  Value search(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
    ...
    else
    {
        ...
        tte->save(posKey, VALUE_NONE, ss->ttPv, BOUND_NONE, DEPTH_NONE, MOVE_NONE, eval, thisThread->optimism[us]);
    }
            ...
            if (move != excludedMove && pos.legal(move))
            {
                ...
                if (value >= probCutBeta)
                {
                    ...
                    tte->save(posKey, value_to_tt(value, ss->ply), ss->ttPv, BOUND_LOWER, depth - 3, move,
                              ss->staticEval, thisThread->optimism[us]);
                    return value;
                }
            }
    }
...
moves_loop: // When in check, search starts here
  ...
  Value qsearch(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth) {
    ...
    else
    {
        if (ss->ttHit)
        {
            ...
            if ((ss->staticEval = bestValue = tte->eval()) == VALUE_NONE
                || abs(tte->optimism() - thisThread->optimism[pos.side_to_move()]) > 10)
        ...
        if (bestValue >= beta)
        {
            ...
            if (!ss->ttHit)
                tte->save(posKey, value_to_tt(bestValue, ss->ply), false, BOUND_LOWER,
                          DEPTH_NONE, MOVE_NONE, ss->staticEval, thisThread->optimism[pos.side_to_move()]);
    ...
    tte->save(posKey, value_to_tt(bestValue, ss->ply), pvHit,
              ...
              ttDepth, bestMove, ss->staticEval, thisThread->optimism[pos.side_to_move()]);
