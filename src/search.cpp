...
Value futility_margin(Depth d, bool noTtCutNode, bool improving, bool oppWorsening) {
    Value futilityMult       = 122 - 33 * noTtCutNode;
...
int stat_bonus(Depth d) { return std::min(185 * d - 104, 1606); }

// History and stats update malus, based on depth
int stat_malus(Depth d) { return std::min(824 * d - 261, 2210); }
...
void Search::Worker::iterative_deepening() {
           ...
           && !(limits.depth && mainThread && rootDepth > limits.depth))
    {
        ...
        for (pvIdx = 0; pvIdx < multiPV; ++pvIdx)
        {
            ...
            delta     = 4 + avg * avg / 12549;
            alpha     = std::max(avg - delta, -VALUE_INFINITE);
            beta      = std::min(avg + delta, VALUE_INFINITE);

            // Adjust optimism based on root move's averageScore (~4 Elo)
            optimism[us]  = 135 * avg / (std::abs(avg) + 86);
...
Value Search::Worker::search(
  Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
    ...
    {
        int bonus = std::clamp(-11 * int((ss - 1)->staticEval + ss->staticEval), -1672, 1444) + 788;
    ...
    if (eval < alpha - 497 - 282 * depth * depth)
    {
    ...
    if (cutNode && (ss - 1)->currentMove != Move::null() && (ss - 1)->statScore < 13868
        && eval >= beta && ss->staticEval >= beta - 23 * depth + 404 && !excludedMove
    ...
    probCutBeta = beta + 188 - 52 * improving;
    if (!PvNode && depth > 3
...
moves_loop:  // When in check, search starts here
    ...
    while ((move = mp.next_move(moveCountPruning)) != Move::none())
    {
        ...
        if (!rootNode && pos.non_pawn_material(us) && bestValue > VALUE_TB_LOSS_IN_MAX_PLY)
        {
            ...
            if (capture || givesCheck)
            {
                ...
                {
                    Value futilityValue = ss->staticEval + 296 + 250 * lmrDepth
                ...
                int seeHist = std::clamp(captHist / 32, -157 * depth, 159 * depth);
            ...
            else
            {
                ...
                if (history < -4131 * depth)
                    continue;

                history += 2 * thisThread->mainHistory[us][move.from_to()];

                lmrDepth += history / 3818;

                Value futilityValue =
                  ss->staticEval + (bestValue < ss->staticEval - 51 ? 148 : 52) + 140 * lmrDepth;
        ...
        if (ss->ply < thisThread->rootDepth * 2)
        {
            ...
            if (!rootNode && move == ttData.move && !excludedMove
                && depth >= 4 - (thisThread->completedDepth > 37) + ss->ttPv
                ...
                && ttData.depth >= depth - 3)
            {
                ...
                {
                    int doubleMargin = 262 * PvNode - 200 * !ttCapture;
                    int tripleMargin = 99 + 265 * PvNode - 256 * !ttCapture + 94 * ss->ttPv;
        ...
        ss->statScore = 2 * thisThread->mainHistory[us][move.from_to()]
                      + (*contHist[0])[movedPiece][move.to_sq()]
                      + (*contHist[1])[movedPiece][move.to_sq()] - 4412;

        // Decrease/increase reduction for moves with a good/bad history (~8 Elo)
        r -= ss->statScore / 10602;
    ...
    {
        int bonus = (121 * (depth > 5) + 37 * (PvNode || cutNode) + 173 * ((ss - 1)->moveCount > 8)
                     + 110 * (!ss->inCheck && bestValue <= ss->staticEval - 101)
                     + 133 * (!(ss - 1)->inCheck && bestValue <= -(ss - 1)->staticEval - 91));

        // Proportional to "how much damage we have to undo"
        bonus += std::clamp(-(ss - 1)->statScore / 101, -93, 301);
...
Depth Search::Worker::reduction(bool i, Depth d, int mn, int delta) const {
    ...
    return (reductionScale + 1283 - delta * 812 / rootDelta) / 1024 + (!i && reductionScale > 1311);
}
