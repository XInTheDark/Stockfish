...
namespace {
  ...
  Depth reduction(bool i, Depth d, int mn, Value delta, Value rootDelta) {
    ...
    return (r + 1527 - int(delta) * 1240 / int(rootDelta)) / 1024 + (!i && r > 894);
  }
...
void Search::init() {
  ...
  for (int i = 1; i < MAX_MOVES; ++i)
      Reductions[i] = int((19.13 + std::log(Threads.size()) / 2) * std::log(i));
}
...
void Thread::search() {
         ...
         && !(Limits.depth && mainThread && rootDepth > Limits.depth))
  {
      ...
      for (pvIdx = 0; pvIdx < multiPV && !Threads.stop; ++pvIdx)
      {
          ...
          delta = Value(9) + int(prev) * prev / 15454;
          alpha = std::max(prev - delta,-VALUE_INFINITE);
          beta  = std::min(prev + delta, VALUE_INFINITE);

          // Adjust optimism based on root move's previousScore
          int opt = 102 * prev / (std::abs(prev) + 126);
...
namespace {
  ...
  Value search(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
    ...
    {
        int bonus = std::clamp(-23 * int((ss-1)->staticEval + ss->staticEval), -1969, 1969);
    ...
    // return a fail low.
    if (eval < alpha - 454 - 259 * depth * depth)
    {
    ...
    if (   !ss->ttPv
        &&  depth < 10
        &&  eval - futility_margin(depth, cutNode && !ss->ttHit, improving) - (ss-1)->statScore / 298 >= beta
        &&  eval >= beta
        &&  eval < 30691) // larger than VALUE_KNOWN_WIN, but smaller than TB wins
        return eval;

    // Step 9. Null move search with verification search (~35 Elo)
    if (   !PvNode
        && (ss-1)->currentMove != MOVE_NULL
        && (ss-1)->statScore < 16231
        &&  eval >= beta
        &&  eval >= ss->staticEval
        &&  ss->staticEval >= beta - 27 * depth + 286
        && !excludedMove
        ...
        &&  beta > VALUE_TB_LOSS_IN_MAX_PLY)
    {
        ...
        Depth R = std::min(int(eval - beta) / 145, 6) + depth / 3 + 4;
    ...
    probCutBeta = beta + 186 - 63 * improving;
...
moves_loop: // When in check, search starts here
    ...
    while ((move = mp.next_move(moveCountPruning)) != MOVE_NONE)
    {
          ...
          && bestValue > VALUE_TB_LOSS_IN_MAX_PLY)
      {
              ...
              || givesCheck)
          {
              ...
              if (   !givesCheck
                  && lmrDepth < 8
                  && !ss->inCheck
                  && ss->staticEval + 169 + 263 * lmrDepth + PieceValue[EG][pos.piece_on(to_sq(move))]
                   + captureHistory[movedPiece][to_sq(move)][type_of(pos.piece_on(to_sq(move)))] / 7 < alpha)
                  continue;

              Bitboard occupied;
              // SEE based pruning (~11 Elo)
              if (!pos.see_ge(move, occupied, Value(-192) * depth))
              {
                 if (depth < 2 - capture)
                    continue;
          ...
          else
          {
              ...
              lmrDepth += history / 6772;
              lmrDepth = std::max(lmrDepth, -2);

              // Futility pruning: parent node (~13 Elo)
              if (   !ss->inCheck
                  && lmrDepth < 12
                  && ss->staticEval + 98 + 128 * lmrDepth <= alpha)
                  continue;

              lmrDepth = std::max(lmrDepth, 0);

              // Prune moves with negative SEE (~4 Elo)
              if (!pos.see_ge(move, Value(-21 * lmrDepth * lmrDepth - 20 * lmrDepth)))
                  continue;
          }
      }
      ...
      if (ss->ply < thisThread->rootDepth * 2)
      {
          ...
          if (   !rootNode
           ...
           /* &&  ttValue != VALUE_NONE Already implicit in the next condition */
              ...
              &&  tte->depth() >= depth - 3)
          {
              Value singularBeta = ttValue - (74 + 70 * (ss->ttPv && !PvNode)) * depth / 64;
              ...
              // If we are on a cutNode, reduce it based on depth (negative extension) (~1 Elo)
                  ...
                  extension = depth > 8 && depth < 15 ? -3 : -1;
          ...
          // Quiet ttMove extensions (~1 Elo)
                   ...
                   && move == ttMove
                   && move == ss->killers[0]
                   && (*contHist[0])[movedPiece][to_sq(move)] >= 5028)
              extension = 1;
      }
      ...
      r -= ss->statScore / (9551 + 4380 * (depth > 6 && depth < 23));
              ...
              || (cutNode && (ss-1)->moveCount > 1)))
      {
          ...
          if (value > alpha && d < newDepth)
          {
              ...
              const bool doDeeperSearch = value > (bestValue + 77 + 10 * (newDepth - d));
              const bool doEvenDeeperSearch = value > alpha + 667 && ss->doubleExtensions <= 8;
    ...
    */
    ...
    {
        int bonus = (depth > 5) + (PvNode || cutNode) + (bestValue < alpha - 133 * depth) + ((ss-1)->moveCount > 11);
  ...
  Value qsearch(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth) {
    ...
    while ((move = mp.next_move()) != MOVE_NONE)
    {
        ...
        if (bestValue > VALUE_TB_LOSS_IN_MAX_PLY)
        {
            ...
            if (   !capture
                && (*contHist[0])[pos.moved_piece(move)][to_sq(move)] < 650
                && (*contHist[1])[pos.moved_piece(move)][to_sq(move)] < 759)
                continue;
  ...
  void update_all_stats(const Position& pos, Stack* ss, Move bestMove, Value bestValue, Value beta, Square prevSq,
    ...
    {
        int bestMoveBonus = bestValue > beta + 160 ? quietMoveBonus  // larger bonus
