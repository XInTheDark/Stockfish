...
Value futility_margin(Depth d, bool noTtCutNode, bool improving) {
    return Value((112 - 41 * noTtCutNode) * (d - improving));
}
...
Depth reduction(bool i, Depth d, int mn, Value delta, Value rootDelta) {
    ...
    return (reductionScale + 1407 - int(delta) * 961 / int(rootDelta)) / 1024
         + (!i && reductionScale > 844);
}
...
int stat_bonus(Depth d) { return std::min(337 * d - 489, 1659); }

// History and stats update malus, based on depth
int stat_malus(Depth d) { return std::min(472 * d - 423, 1345); }
...
void Search::init() {
    ...
    for (int i = 1; i < MAX_MOVES; ++i)
        Reductions[i] = int((19.20 + std::log(Threads.size()) / 2) * std::log(i));
}
...
void Thread::search() {
           ...
           && !(Limits.depth && mainThread && rootDepth > Limits.depth))
    {
        ...
        for (pvIdx = 0; pvIdx < multiPV && !Threads.stop; ++pvIdx)
        {
            ...
            delta     = Value(11) + int(avg) * avg / 14782;
            alpha     = std::max(avg - delta, -VALUE_INFINITE);
            beta      = std::min(avg + delta, VALUE_INFINITE);

            // Adjust optimism based on root move's averageScore (~4 Elo)
            optimism[us]  = 108 * (avg + 38) / (std::abs(avg + 29) + 121);
            optimism[~us] = -111 * (avg + 41) / (std::abs(avg + 14) + 119);
...
Value search(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
    ...
    {
        int bonus = std::clamp(-18 * int((ss - 1)->staticEval + ss->staticEval), -1716, 1716);
    ...
    if (eval < alpha - 445 - (265 - 156 * ((ss + 1)->cutoffCnt > 3)) * depth * depth)
    {
    ...
    if (!ss->ttPv && depth < 8
        && eval - futility_margin(depth, cutNode && !ss->ttHit, improving)
               - (ss - 1)->statScore / 337
             >= beta
        && eval >= beta && eval < 25831  // smaller than TB wins
        && (!ttMove || ttCapture))
        return eval;

    // Step 9. Null move search with verification search (~35 Elo)
    if (!PvNode && (ss - 1)->currentMove != MOVE_NULL && (ss - 1)->statScore < 17853 && eval >= beta
        && eval >= ss->staticEval && ss->staticEval >= beta - 20 * depth + 284 && !excludedMove
    ...
    // if depth is not too low.
    if (cutNode && depth >= 7 && !ttMove)
        depth -= 2;

    probCutBeta = beta + 168 - 79 * improving;
...
moves_loop:  // When in check, search starts here
    ...
    while ((move = mp.next_move(moveCountPruning)) != MOVE_NONE)
    {
        ...
        if (!rootNode && pos.non_pawn_material(us) && bestValue > VALUE_TB_LOSS_IN_MAX_PLY)
        {
            ...
            if (capture || givesCheck)
            {
                ...
                if (!givesCheck && lmrDepth < 6 && !ss->inCheck)
                {
                    ...
                    int   futilityEval =
                      ss->staticEval + 239 + 277 * lmrDepth + PieceValue[capturedPiece]
                ...
                if (!pos.see_ge(move, Value(-197) * depth))
                    continue;
            }
            else
            {
                ...
                if (lmrDepth < 6 && history < -4006 * depth)
                    continue;

                history += 2 * thisThread->mainHistory[us][from_to(move)];

                lmrDepth += history / 7355;
                lmrDepth = std::max(lmrDepth, -1);

                // Futility pruning: parent node (~13 Elo)
                if (!ss->inCheck && lmrDepth < 12
                    && ss->staticEval + (bestValue < ss->staticEval - 63 ? 130 : 84)
                           + 119 * lmrDepth
                         <= alpha)
                    continue;
        ...
        if (ss->ply < thisThread->rootDepth * 2)
        {
            ...
            if (!rootNode && move == ttMove && !excludedMove
                ...
                && tte->depth() >= depth - 3)
            {
                Value singularBeta  = ttValue - (62 + 58 * (ss->ttPv && !PvNode)) * depth / 64;
            ...
            // Quiet ttMove extensions (~1 Elo)
                     ...
                     && (*contHist[0])[movedPiece][to_sq(move)] >= 4182)
                extension = 1;
        }
        ...
        ss->statScore = 2 * thisThread->mainHistory[us][from_to(move)]
                      ...
                      + (*contHist[1])[movedPiece][to_sq(move)]
                      + (*contHist[3])[movedPiece][to_sq(move)] - 3881;

        // Decrease/increase reduction for moves with a good/bad history (~25 Elo)
        r -= ss->statScore / (11517 + 3540 * (depth > 6 && depth < 21));
            ...
            && (!ss->ttPv || !capture || (cutNode && (ss - 1)->moveCount > 1)))
        {
            ...
            if (value > alpha && d < newDepth)
            {
                ...
                const bool doEvenDeeperSearch = value > alpha + 671 && ss->doubleExtensions <= 6;
        ...
        if (value > bestValue)
        {
            ...
            if (value > alpha)
            {
                ...
                else
                {
                    ...
                    if (depth > 2 && depth < 12 && beta < 13897 && value > -11989)
                        depth -= 2;
    ...
    {
        int bonus = (depth > 6) + (PvNode || cutNode) + (bestValue < alpha - 615)
                  + ((ss - 1)->moveCount > 10);
        update_continuation_histories(ss - 1, pos.piece_on(prevSq), prevSq,
...
Value qsearch(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth) {
    ...
    else
    {
        ...
        futilityBase = ss->staticEval + 205;
    }
...
void update_all_stats(const Position& pos,
    ...
    {
        int bestMoveBonus = bestValue > beta + 153 ? quietMoveBonus      // larger bonus
