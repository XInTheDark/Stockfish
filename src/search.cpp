...
void Thread::search() {
         ...
         && !(Limits.depth && mainThread && rootDepth > Limits.depth))
  {
      ...
      {
          double fallingEval = (75 + 14 * (mainThread->bestPreviousAverageScore - bestValue)
                                    +  6 * (mainThread->iterValue[iterIdx] - bestValue)) * 111 / 61960;
          fallingEval = std::clamp(fallingEval, 0.389, 1.734);

          // If the bestMove is stable over several iterations, reduce time accordingly
          timeReduction = lastBestMoveDepth + 9 < completedDepth ? 1.664 : 0.539;
          double reduction = (1.583 + mainThread->previousTimeReduction) / (2.106 * timeReduction);
          double bestMoveInstability = 0.975 + 2.031 * totBestMoveChanges / Threads.size();
