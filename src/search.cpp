...
void Search::Worker::iterative_deepening() {
           ...
           && !(limits.depth && mainThread && rootDepth > limits.depth))
    {
        ...
        if (limits.use_time_management() && !threads.stop && !mainThread->stopOnPonderhit)
        {
            ...
            double fallingEval = (1094 + 224 * (mainThread->bestPreviousAverageScore - bestValue)
                                  + 102 * (mainThread->iterValue[iterIdx] - bestValue))
                               / 10000.0;
            fallingEval = std::clamp(fallingEval, 0.490, 1.523);

            // If the bestMove is stable over several iterations, reduce time accordingly
            timeReduction    = lastBestMoveDepth + 7 < completedDepth ? 1.647 : 0.724;
            double reduction = (1.44 + mainThread->previousTimeReduction) / (2.85 * timeReduction);
            double bestMoveInstability = 1 + 1.84 * totBestMoveChanges / threads.size();
            double recapture           = limits.capSq == rootMoves[0].pv[0].to_sq() ? 0.968 : 0.801;
            ...
            if (completedDepth >= 11 && nodesEffort >= 82 && elapsedTime > totalTime * 0.759
                && !mainThread->ponder)
                threads.stop = true;
