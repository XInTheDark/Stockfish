...
Value futility_margin(Depth d, bool noTtCutNode, bool improving, bool oppWorsening) {
    Value futilityMult       = 133 - 41 * noTtCutNode;
    Value improvingDeduction = 56 * improving * futilityMult / 32;
    Value worseningDeduction = 374 * oppWorsening * futilityMult / 1024;
...
Value to_corrected_static_eval(Value v, const Worker& w, const Position& pos) {
    ...
    v += cv * std::abs(cv) / 5819;
    return std::clamp(v, VALUE_TB_LOSS_IN_MAX_PLY + 1, VALUE_TB_WIN_IN_MAX_PLY - 1);
}

// History and stats update bonus, based on depth
int stat_bonus(Depth d) { return std::clamp(200 * d - 281, 17, 1764); }

// History and stats update malus, based on depth
int stat_malus(Depth d) { return (d < 4 ? 806 * d - 281 : 1901); }
...
void Search::Worker::iterative_deepening() {
           ...
           && !(limits.depth && mainThread && rootDepth > limits.depth))
    {
        ...
        for (pvIdx = 0; pvIdx < multiPV && !threads.stop; ++pvIdx)
        {
            ...
            delta     = 9 + avg * avg / 10436;
            alpha     = std::max(avg - delta, -VALUE_INFINITE);
            beta      = std::min(avg + delta, VALUE_INFINITE);

            // Adjust optimism based on root move's averageScore (~4 Elo)
            optimism[us]  = 140 * avg / (std::abs(avg) + 99);
...
void Search::Worker::clear() {
    ...
    for (size_t i = 1; i < reductions.size(); ++i)
        reductions[i] = int((19.06 + std::log(size_t(options["Threads"])) / 2) * std::log(i));
...
Value Search::Worker::search(
  Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
    ...
    {
        int bonus = std::clamp(-11 * int((ss - 1)->staticEval + ss->staticEval), -1566, 1342);
    ...
    // return a fail low.
    if (eval < alpha - 453 - 313 * depth * depth)
    {
    ...
    if (!PvNode && (ss - 1)->currentMove != Move::null() && (ss - 1)->statScore < 14956
        && eval >= beta && ss->staticEval >= beta - 22 * depth + 387 && !excludedMove
        ...
        && beta > VALUE_TB_LOSS_IN_MAX_PLY)
    {
        ...
        Depth R = std::min(int(eval - beta) / 181, 6) + depth / 3 + 5;
    ...
    probCutBeta = beta + 180 - 51 * improving;
    if (
      !PvNode && depth > 3
...
moves_loop:  // When in check, search starts here
    ...
    while ((move = mp.next_move(moveCountPruning)) != Move::none())
    {
        ...
        if (!rootNode && pos.non_pawn_material(us) && bestValue > VALUE_TB_LOSS_IN_MAX_PLY)
        {
            ...
            if (capture || givesCheck)
            {
                ...
                {
                    Value futilityValue = ss->staticEval + 248 + 261 * lmrDepth
                ...
                int seeHist = std::clamp(captHist * 81 / 2048, -173 * depth, 142 * depth);
                if (!pos.see_ge(move, -181 * depth - seeHist))
                    continue;
            }
            else
            {
                ...
                if (lmrDepth < 6 && history < -4303 * depth)
                    continue;

                history += 2 * thisThread->mainHistory[us][move.from_to()];

                lmrDepth += history / 3538;

                Value futilityValue =
                  ss->staticEval + (bestValue < ss->staticEval - 48 ? 162 : 54) + 130 * lmrDepth;
        ...
        if (ss->ply < thisThread->rootDepth * 2)
        {
            ...
            if (!rootNode && move == ttMove && !excludedMove
                && depth >= 4 - (thisThread->completedDepth > 39) + ss->ttPv
                && std::abs(ttValue) < VALUE_TB_WIN_IN_MAX_PLY && (tte->bound() & BOUND_LOWER)
                && tte->depth() >= depth - 3)
            {
                Value singularBeta  = ttValue - (54 + 74 * (ss->ttPv && !PvNode)) * depth / 64;
                ...
                {
                    int doubleMargin = 284 * PvNode - 215 * !ttCapture;
                    int tripleMargin = 120 + 292 * PvNode - 292 * !ttCapture + 112 * ss->ttPv;
        ...
        ss->statScore = 2 * thisThread->mainHistory[us][move.from_to()]
                      + (*contHist[0])[movedPiece][move.to_sq()]
                      + (*contHist[1])[movedPiece][move.to_sq()] - 5243;

        // Decrease/increase reduction for moves with a good/bad history (~8 Elo)
        r -= ss->statScore / (13152 - std::min(depth, 12) * 118);
        ...
        if (value > bestValue)
        {
            ...
            if (value > alpha)
            {
                ...
                else
                {
                    ...
                    if (depth > 2 && depth < 12 && std::abs(value) < VALUE_TB_WIN_IN_MAX_PLY)
                        depth -= 2;
    ...
    {
        int bonus = (135 * (depth > 5) + 110 * (PvNode || cutNode)
                     + 206 * ((ss - 1)->statScore < -11425) + 129 * ((ss - 1)->moveCount > 10)
                     + 58 * (!ss->inCheck && bestValue <= ss->staticEval - 122)
                     + 144 * (!(ss - 1)->inCheck && bestValue <= -(ss - 1)->staticEval - 77));
        update_continuation_histories(ss - 1, pos.piece_on(prevSq), prevSq,
...
Value Search::Worker::qsearch(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth) {
    ...
    else
    {
        ...
        futilityBase = ss->staticEval + 271;
    }
...
Depth Search::Worker::reduction(bool i, Depth d, int mn, int delta) {
    ...
    return (reductionScale + 1334 - delta * 665 / rootDelta) / 1024 + (!i && reductionScale > 1184);
}
