...
Value futility_margin(Depth d, bool noTtCutNode, bool improving, bool oppWorsening) {
    Value futilityMult       = 130 - 47 * noTtCutNode;
    Value improvingDeduction = 57 * improving * futilityMult / 32;
    Value worseningDeduction = (314 + 52 * improving) * oppWorsening * futilityMult / 1024;
...
Value to_corrected_static_eval(Value v, const Worker& w, const Position& pos) {
    ...
    v += cv * std::abs(cv) / 7037;
    return std::clamp(v, VALUE_TB_LOSS_IN_MAX_PLY + 1, VALUE_TB_WIN_IN_MAX_PLY - 1);
}

// History and stats update bonus, based on depth
int stat_bonus(Depth d) { return std::clamp(203 * d - 291, 16, 1437); }

// History and stats update malus, based on depth
int stat_malus(Depth d) { return (d < 4 ? 569 * d - 295 : 1581); }
...
void Search::Worker::iterative_deepening() {
           ...
           && !(limits.depth && mainThread && rootDepth > limits.depth))
    {
        ...
        for (pvIdx = 0; pvIdx < multiPV && !threads.stop; ++pvIdx)
        {
            ...
            delta     = 10 + avg * avg / 9205;
            alpha     = std::max(avg - delta, -VALUE_INFINITE);
            beta      = std::min(avg + delta, VALUE_INFINITE);

            // Adjust optimism based on root move's averageScore (~4 Elo)
            optimism[us]  = 118 * avg / (std::abs(avg) + 87);
...
void Search::Worker::clear() {
    ...
    for (size_t i = 1; i < reductions.size(); ++i)
        reductions[i] = int((20.29 + std::log(size_t(options["Threads"])) / 2) * std::log(i));
...
Value Search::Worker::search(
  Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
    ...
    {
        int bonus = std::clamp(-12 * int((ss - 1)->staticEval + ss->staticEval), -1768, 1657);
    ...
    if (eval < alpha - 455 - (317 - 134 * ((ss + 1)->cutoffCnt > 3)) * depth * depth)
    {
    ...
    if (!PvNode && (ss - 1)->currentMove != Move::null() && (ss - 1)->statScore < 16876
        && eval >= beta && ss->staticEval >= beta - 20 * depth + 329 && !excludedMove
        ...
        && beta > VALUE_TB_LOSS_IN_MAX_PLY)
    {
        ...
        Depth R = std::min(int(eval - beta) / 135, 6) + depth / 3 + 4;
    ...
    probCutBeta = beta + 174 - 66 * improving;
    if (
      !PvNode && depth > 3
...
moves_loop:  // When in check, search starts here
    ...
    while ((move = mp.next_move(moveCountPruning)) != Move::none())
    {
        ...
        if (!rootNode && pos.non_pawn_material(us) && bestValue > VALUE_TB_LOSS_IN_MAX_PLY)
        {
            ...
            if (capture || givesCheck)
            {
                ...
                {
                    Value futilityValue = ss->staticEval + 304 + 268 * lmrDepth
                ...
                int seeHist = std::clamp(captHist / 32, -194 * depth, 188 * depth);
                if (!pos.see_ge(move, -177 * depth - seeHist))
                    continue;
            }
            else
            {
                ...
                if (lmrDepth < 6 && history < -4304 * depth)
                    continue;

                history += 2 * thisThread->mainHistory[us][move.from_to()];

                lmrDepth += history / 4669;

                Value futilityValue =
                  ss->staticEval + (bestValue < ss->staticEval - 53 ? 139 : 55) + 136 * lmrDepth;
        ...
        if (ss->ply < thisThread->rootDepth * 2)
        {
                ...
                && tte->depth() >= depth - 3)
            {
                ...
                {
                    int doubleMargin = 273 * PvNode - 215 * !ttCapture;
                    int tripleMargin =
                      130 + 235 * PvNode - 246 * !ttCapture + 124 * (ss->ttPv || !ttCapture);
                    int quadMargin = 463 + 344 * PvNode - 292 * !ttCapture + 201 * ss->ttPv;
        ...
        ss->statScore = 2 * thisThread->mainHistory[us][move.from_to()]
                      + (*contHist[0])[movedPiece][move.to_sq()]
                      + (*contHist[1])[movedPiece][move.to_sq()] - 5143;

        // Decrease/increase reduction for moves with a good/bad history (~8 Elo)
        r -= ss->statScore / (16590 - std::min(depth, 15) * 100);
        ...
        if (value > bestValue)
        {
            ...
            if (value > alpha)
            {
                ...
                else
                {
                    ...
                    if (depth > 2 && depth < 14 && abs(value) < VALUE_TB_WIN_IN_MAX_PLY)
                        depth -= 2;
    ...
    {
        int bonus = (depth > 5) + (PvNode || cutNode) + ((ss - 1)->statScore < -13650)
                  + ((ss - 1)->moveCount > 10) + (!ss->inCheck && bestValue <= ss->staticEval - 135)
...
Value Search::Worker::qsearch(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth) {
    ...
    else
    {
        ...
        futilityBase = ss->staticEval + 263;
    }
...
Depth Search::Worker::reduction(bool i, Depth d, int mn, int delta) {
    ...
    return (reductionScale + 1311 - delta * 757 / rootDelta) / 1024 + (!i && reductionScale > 1089);
}
