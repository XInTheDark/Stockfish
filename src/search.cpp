...
Value futility_margin(Depth d, bool noTtCutNode, bool improving, bool oppWorsening) {
    Value futilityMult       = 127 - 44 * noTtCutNode;
    Value improvingDeduction = 54 * improving * futilityMult / 32;
    Value worseningDeduction = (311 + 52 * improving) * oppWorsening * futilityMult / 1024;
...
Value to_corrected_static_eval(Value v, const Worker& w, const Position& pos) {
    ...
    v += cv * std::abs(cv) / 8901;
    return std::clamp(v, VALUE_TB_LOSS_IN_MAX_PLY + 1, VALUE_TB_WIN_IN_MAX_PLY - 1);
}

// History and stats update bonus, based on depth
int stat_bonus(Depth d) { return std::clamp(202 * d - 286, 15, 1297); }

// History and stats update malus, based on depth
int stat_malus(Depth d) { return (d < 4 ? 604 * d - 303 : 1404); }
...
void Search::Worker::iterative_deepening() {
           ...
           && !(limits.depth && mainThread && rootDepth > limits.depth))
    {
        ...
        for (pvIdx = 0; pvIdx < multiPV && !threads.stop; ++pvIdx)
        {
            ...
            delta     = 10 + avg * avg / 10236;
            alpha     = std::max(avg - delta, -VALUE_INFINITE);
            beta      = std::min(avg + delta, VALUE_INFINITE);

            // Adjust optimism based on root move's averageScore (~4 Elo)
            optimism[us]  = 122 * avg / (std::abs(avg) + 95);
...
void Search::Worker::clear() {
    ...
    for (size_t i = 1; i < reductions.size(); ++i)
        reductions[i] = int((19.31 + std::log(size_t(options["Threads"])) / 2) * std::log(i));
...
Value Search::Worker::search(
  Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
    ...
    {
        int bonus = std::clamp(-13 * int((ss - 1)->staticEval + ss->staticEval), -1726, 1456);
    ...
    if (eval < alpha - 457 - (292 - 147 * ((ss + 1)->cutoffCnt > 3)) * depth * depth)
    {
    ...
    if (!PvNode && (ss - 1)->currentMove != Move::null() && (ss - 1)->statScore < 18484
        && eval >= beta && ss->staticEval >= beta - 19 * depth + 334 && !excludedMove
    ...
    probCutBeta = beta + 156 - 67 * improving;
    if (
      !PvNode && depth > 3
...
moves_loop:  // When in check, search starts here
    ...
    while ((move = mp.next_move(moveCountPruning)) != Move::none())
    {
        ...
        if (!rootNode && pos.non_pawn_material(us) && bestValue > VALUE_TB_LOSS_IN_MAX_PLY)
        {
            ...
            if (capture || givesCheck)
            {
                ...
                {
                    Value futilityValue = ss->staticEval + 287 + 277 * lmrDepth
                ...
                int seeHist = std::clamp(captHist / 32, -200 * depth, 192 * depth);
                if (!pos.see_ge(move, -180 * depth - seeHist))
                    continue;
            }
            else
            {
                ...
                if (lmrDepth < 6 && history < -4242 * depth)
                    continue;

                history += 2 * thisThread->mainHistory[us][move.from_to()];

                lmrDepth += history / 5046;

                Value futilityValue =
                  ss->staticEval + (bestValue < ss->staticEval - 51 ? 128 : 57) + 140 * lmrDepth;
        ...
        if (value > bestValue)
        {
            ...
            if (value > alpha)
            {
                ...
                else
                {
                    ...
                    if (depth > 2 && depth < 13 && beta < 15044 && value > -14299)
                        depth -= 2;
    ...
    {
        int bonus = (depth > 5) + (PvNode || cutNode) + ((ss - 1)->statScore < -14049)
                  + ((ss - 1)->moveCount > 10)
                  + (!ss->inCheck && bestValue <= ss->staticEval - 135);
        update_continuation_histories(ss - 1, pos.piece_on(prevSq), prevSq,
...
Depth Search::Worker::reduction(bool i, Depth d, int mn, int delta) {
    ...
    return (reductionScale + 1288 - delta * 757 / rootDelta) / 1024 + (!i && reductionScale > 1110);
}
