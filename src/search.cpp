...
namespace {
  ...
  Value futility_margin(Depth d, bool noTtCutNode, bool improving) {
    return Value((153 - 39 * noTtCutNode) * (d - improving));
  }
  ...
  Depth reduction(bool i, Depth d, int mn, Value delta, Value rootDelta) {
    ...
    return  (reductionScale + 1619 - int(delta) * 1137 / int(rootDelta)) / 1024
          + (!i && reductionScale > 970);
  }
...
void Search::init() {
  ...
  for (int i = 1; i < MAX_MOVES; ++i)
      Reductions[i] = int((21.75 + std::log(Threads.size()) / 2) * std::log(i));
}
...
void Thread::search() {
         ...
         && !(Limits.depth && mainThread && rootDepth > Limits.depth))
  {
      ...
      for (pvIdx = 0; pvIdx < multiPV && !Threads.stop; ++pvIdx)
      {
          ...
          delta = Value(10) + int(prev) * prev / 16008;
          alpha = std::max(prev - delta,-VALUE_INFINITE);
          beta  = std::min(prev + delta, VALUE_INFINITE);

          // Adjust optimism based on root move's previousScore
          int opt = 114 * prev / (std::abs(prev) + 149);
...
namespace {
  ...
  Value search(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
    ...
    {
        int bonus = std::clamp(-35 * int((ss-1)->staticEval + ss->staticEval) / 2, -1840, 1840);
    ...
    // return a fail low.
    if (eval < alpha - 456 - 236 * depth * depth)
    {
    ...
    if (   !ss->ttPv
        ...
        &&  eval >= beta
        &&  eval < 24444) // larger than VALUE_KNOWN_WIN, but smaller than TB wins
        return eval;

    // Step 9. Null move search with verification search (~35 Elo)
    if (   !PvNode
        && (ss-1)->currentMove != MOVE_NULL
        && (ss-1)->statScore < 16819
        &&  eval >= beta
        &&  eval >= ss->staticEval
        &&  ss->staticEval >= beta - 21 * depth + 239
        && !excludedMove
        ...
        &&  beta > VALUE_TB_LOSS_IN_MAX_PLY)
    {
        ...
        Depth R = std::min(int(eval - beta) / 189, 6) + depth / 3 + 4;
    ...
    probCutBeta = beta + 187 - 53 * improving;
...
moves_loop: // When in check, search starts here
    ...
    while ((move = mp.next_move(moveCountPruning)) != MOVE_NONE)
    {
          ...
          && bestValue > VALUE_TB_LOSS_IN_MAX_PLY)
      {
              ...
              || givesCheck)
          {
              ...
              if (   !givesCheck
                  && lmrDepth < 7
                  && !ss->inCheck
                  && ss->staticEval + 180 + 236 * lmrDepth + PieceValue[pos.piece_on(to_sq(move))]
          ...
          else
          {
              ...
              lmrDepth += history / 6386;
              lmrDepth = std::max(lmrDepth, -2);

              // Futility pruning: parent node (~13 Elo)
              if (   !ss->inCheck
                  && lmrDepth < 12
                  && ss->staticEval + 111 + 147 * lmrDepth <= alpha)
                  continue;
      ...
      if (ss->ply < thisThread->rootDepth * 2)
      {
          ...
          // Quiet ttMove extensions (~1 Elo)
                   ...
                   && move == ttMove
                   && move == ss->killers[0]
                   && (*contHist[0])[movedPiece][to_sq(move)] >= 5455)
              extension = 1;
      }
      ...
      r -= ss->statScore / (10687 + 4461 * (depth > 5 && depth < 25));
              ...
              || (cutNode && (ss-1)->moveCount > 1)))
      {
          ...
          if (value > alpha && d < newDepth)
          {
              ...
              const bool doDeeperSearch = value > (bestValue + 60 + 12 * (newDepth - d));
              const bool doEvenDeeperSearch = value > alpha + 680 && ss->doubleExtensions <= 6;
    ...
    */
    ...
    {
        int bonus = (depth > 5) + (PvNode || cutNode) + (bestValue < alpha - 758) + ((ss-1)->moveCount > 12);
  ...
  Value qsearch(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth) {
    ...
    else
    {
        ...
        futilityBase = std::min(ss->staticEval, bestValue) + 209;
    }
  ...
  void update_all_stats(const Position& pos, Stack* ss, Move bestMove, Value bestValue, Value beta, Square prevSq,
    ...
    {
        int bestMoveBonus = bestValue > beta + 127 ? quietMoveBonus  // larger bonus
