...
Value futility_margin(Depth d, bool noTtCutNode, bool improving, bool oppWorsening) {
    Value futilityMult       = 122 - 40 * noTtCutNode;
    Value improvingDeduction = 59 * improving * futilityMult / 32;
    Value worseningDeduction = 324 * oppWorsening * futilityMult / 1024;
...
Value to_corrected_static_eval(Value v, const Worker& w, const Position& pos) {
    ...
    v += cv * std::abs(cv) / 5587;
    return std::clamp(v, VALUE_TB_LOSS_IN_MAX_PLY + 1, VALUE_TB_WIN_IN_MAX_PLY - 1);
}

// History and stats update bonus, based on depth
int stat_bonus(Depth d) { return std::clamp(207 * d - 272, 18, 1630); }

// History and stats update malus, based on depth
int stat_malus(Depth d) { return (d < 4 ? 806 * d - 272 : 2027); }
...
void Search::Worker::iterative_deepening() {
           ...
           && !(limits.depth && mainThread && rootDepth > limits.depth))
    {
        ...
        for (pvIdx = 0; pvIdx < multiPV && !threads.stop; ++pvIdx)
        {
            ...
            delta     = 9 + avg * avg / 10351;
            alpha     = std::max(avg - delta, -VALUE_INFINITE);
            beta      = std::min(avg + delta, VALUE_INFINITE);

            // Adjust optimism based on root move's averageScore (~4 Elo)
            optimism[us]  = 131 * avg / (std::abs(avg) + 98);
...
void Search::Worker::clear() {
    ...
    for (size_t i = 1; i < reductions.size(); ++i)
        reductions[i] = int((21.49 + std::log(size_t(options["Threads"])) / 2) * std::log(i));
...
Value Search::Worker::search(
  Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
    ...
    {
        int bonus = std::clamp(-12 * int((ss - 1)->staticEval + ss->staticEval), -1544, 1400);
    ...
    // return a fail low.
    if (eval < alpha - 470 - 328 * depth * depth)
    {
    ...
    if (!PvNode && (ss - 1)->currentMove != Move::null() && (ss - 1)->statScore < 13565
        && eval >= beta && ss->staticEval >= beta - 21 * depth + 372 && !excludedMove
        ...
        && beta > VALUE_TB_LOSS_IN_MAX_PLY)
    {
        ...
        Depth R = std::min(int(eval - beta) / 170, 6) + depth / 3 + 5;
    ...
    probCutBeta = beta + 169 - 58 * improving;
    if (
      !PvNode && depth > 3
...
moves_loop:  // When in check, search starts here
    ...
    while ((move = mp.next_move(moveCountPruning)) != Move::none())
    {
        ...
        if (!rootNode && pos.non_pawn_material(us) && bestValue > VALUE_TB_LOSS_IN_MAX_PLY)
        {
            ...
            if (capture || givesCheck)
            {
                ...
                {
                    Value futilityValue = ss->staticEval + 268 + 239 * lmrDepth
                ...
                int seeHist = std::clamp(captHist / 32, -178 * depth, 154 * depth);
                if (!pos.see_ge(move, -169 * depth - seeHist))
                    continue;
            }
            else
            {
                ...
                if (lmrDepth < 6 && history < -4283 * depth)
                    continue;

                history += 2 * thisThread->mainHistory[us][move.from_to()];

                lmrDepth += history / 3633;

                Value futilityValue =
                  ss->staticEval + (bestValue < ss->staticEval - 47 ? 156 : 57) + 124 * lmrDepth;
        ...
        if (ss->ply < thisThread->rootDepth * 2)
        {
            ...
            if (!rootNode && move == ttMove && !excludedMove
                ...
                && tte->depth() >= depth - 3)
            {
                Value singularBeta  = ttValue - (54 + 61 * (ss->ttPv && !PvNode)) * depth / 64;
                ...
                {
                    int doubleMargin = 293 * PvNode - 209 * !ttCapture;
                    int tripleMargin = 114 + 252 * PvNode - 291 * !ttCapture + 99 * ss->ttPv;
                    int quadMargin   = 545 + 359 * PvNode - 286 * !ttCapture + 221 * ss->ttPv - 164 * !(ss-1)->ttPv;
        ...
        ss->statScore = 2 * thisThread->mainHistory[us][move.from_to()]
                      + (*contHist[0])[movedPiece][move.to_sq()]
                      + (*contHist[1])[movedPiece][move.to_sq()] - 5603;

        // Decrease/increase reduction for moves with a good/bad history (~8 Elo)
        r -= ss->statScore / (12565 - std::min(depth, 12) * 110);
        ...
        if (depth >= 2 && moveCount > 1 + rootNode)
        {
            ...
            if (value > alpha && d < newDepth)
            {
                ...
                const bool doDeeperSearch    = value > (bestValue + 37 + 2 * newDepth);  // (~1 Elo)
    ...
    {
        int bonus = (depth > 4) + (depth > 5) + (PvNode || cutNode) + ((ss - 1)->statScore < -13701)
                  + ((ss - 1)->moveCount > 9) + (!ss->inCheck && bestValue <= ss->staticEval - 113)
                  + (!(ss - 1)->inCheck && bestValue <= -(ss - 1)->staticEval - 83);
        update_continuation_histories(ss - 1, pos.piece_on(prevSq), prevSq,
...
Value Search::Worker::qsearch(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth) {
    ...
    else
    {
        ...
        futilityBase = ss->staticEval + 266;
    }
...
Depth Search::Worker::reduction(bool i, Depth d, int mn, int delta) {
    ...
    return (reductionScale + 1256 - delta * 707 / rootDelta) / 1024 + (!i && reductionScale > 1334);
}
