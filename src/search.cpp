...
Value futility_margin(Depth d, bool noTtCutNode, bool improving, bool oppWorsening) {
    Value futilityMult       = 118 - 35 * noTtCutNode;
...
int stat_bonus(Depth d) { return std::min(185 * d - 106, 1575); }

// History and stats update malus, based on depth
int stat_malus(Depth d) { return std::min(794 * d - 264, 2173); }
...
void Search::Worker::iterative_deepening() {
           ...
           && !(limits.depth && mainThread && rootDepth > limits.depth))
    {
        ...
        for (pvIdx = 0; pvIdx < multiPV; ++pvIdx)
        {
            ...
            delta     = 5 + avg * avg / 12927;
            alpha     = std::max(avg - delta, -VALUE_INFINITE);
            beta      = std::min(avg + delta, VALUE_INFINITE);

            // Adjust optimism based on root move's averageScore (~4 Elo)
            optimism[us]  = 131 * avg / (std::abs(avg) + 89);
...
void Search::Worker::clear() {
    mainHistory.fill(0);
    captureHistory.fill(-709);
    pawnHistory.fill(-1177);
    ...
    for (size_t i = 1; i < reductions.size(); ++i)
        reductions[i] = int((18.84 + std::log(size_t(options["Threads"])) / 2) * std::log(i));
...
Value Search::Worker::search(
  Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
    ...
    {
        int bonus = std::clamp(-10 * int((ss - 1)->staticEval + ss->staticEval), -1731, 1371) + 784;
    ...
    if (eval < alpha - 517 - 277 * depth * depth)
    {
    ...
    if (cutNode && (ss - 1)->currentMove != Move::null() && (ss - 1)->statScore < 14394
        && eval >= beta && ss->staticEval >= beta - 21 * depth + 385 && !excludedMove
        ...
        && beta > VALUE_TB_LOSS_IN_MAX_PLY)
    {
        ...
        Depth R = std::min(int(eval - beta) / 206, 6) + depth / 3 + 5;
    ...
    probCutBeta = beta + 185 - 50 * improving;
    if (!PvNode && depth > 3
...
moves_loop:  // When in check, search starts here
    ...
    while ((move = mp.next_move(moveCountPruning)) != Move::none())
    {
        ...
        if (!rootNode && pos.non_pawn_material(us) && bestValue > VALUE_TB_LOSS_IN_MAX_PLY)
        {
            ...
            if (capture || givesCheck)
            {
                ...
                {
                    Value futilityValue = ss->staticEval + 291 + 255 * lmrDepth
                ...
                int seeHist = std::clamp(captHist / 32, -162 * depth, 164 * depth);
            ...
            else
            {
                ...
                if (history < -4282 * depth)
                    continue;

                history += 2 * thisThread->mainHistory[us][move.from_to()];

                lmrDepth += history / 3915;

                Value futilityValue =
                  ss->staticEval + (bestValue < ss->staticEval - 49 ? 144 : 51) + 137 * lmrDepth;
        ...
        if (ss->ply < thisThread->rootDepth * 2)
        {
            ...
            {
                Value singularBeta  = ttData.value - (53 + 77 * (ss->ttPv && !PvNode)) * depth / 64;
                ...
                {
                    int doubleMargin = 279 * PvNode - 198 * !ttCapture;
                    int tripleMargin = 104 + 266 * PvNode - 275 * !ttCapture + 94 * ss->ttPv;
        ...
        ss->statScore = 2 * thisThread->mainHistory[us][move.from_to()]
                      + (*contHist[0])[movedPiece][move.to_sq()]
                      + (*contHist[1])[movedPiece][move.to_sq()] - 4614;

        // Decrease/increase reduction for moves with a good/bad history (~8 Elo)
        r -= ss->statScore / 10017;
    ...
    {
        int bonus = (118 * (depth > 5) + 38 * (PvNode || cutNode) + 170 * ((ss - 1)->moveCount > 8)
                     + 113 * (!ss->inCheck && bestValue <= ss->staticEval - 97)
                     + 130 * (!(ss - 1)->inCheck && bestValue <= -(ss - 1)->staticEval - 88));

        // Proportional to "how much damage we have to undo"
        bonus += std::clamp(-(ss - 1)->statScore / 103, -94, 303);

        bonus = std::max(bonus, 0);

        update_continuation_histories(ss - 1, pos.piece_on(prevSq), prevSq,
                                      stat_bonus(depth) * bonus / 108);
        thisThread->mainHistory[~us][((ss - 1)->currentMove).from_to()]
          << stat_bonus(depth) * bonus / 182;
...
Depth Search::Worker::reduction(bool i, Depth d, int mn, int delta) const {
    ...
    return (reductionScale + 1321 - delta * 768 / rootDelta) / 1024 + (!i && reductionScale > 1297);
}
