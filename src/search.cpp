...
Value futility_margin(Depth d, bool noTtCutNode, bool improving, bool oppWorsening) {
    Value futilityMult       = 120 - 45 * noTtCutNode;
    Value improvingDeduction = 60 * improving * futilityMult / 32;
    Value worseningDeduction = 317 * oppWorsening * futilityMult / 1024;
...
Value to_corrected_static_eval(Value v, const Worker& w, const Position& pos) {
    ...
    v += cv * std::abs(cv) / 5623;
    return std::clamp(v, VALUE_TB_LOSS_IN_MAX_PLY + 1, VALUE_TB_WIN_IN_MAX_PLY - 1);
}

// History and stats update bonus, based on depth
int stat_bonus(Depth d) { return std::clamp(202 * d - 296, 18, 1538); }

// History and stats update malus, based on depth
int stat_malus(Depth d) { return (d < 4 ? 661 * d - 283 : 1879); }
...
void Search::Worker::iterative_deepening() {
           ...
           && !(limits.depth && mainThread && rootDepth > limits.depth))
    {
        ...
        for (pvIdx = 0; pvIdx < multiPV && !threads.stop; ++pvIdx)
        {
            ...
            delta     = 10 + avg * avg / 9112;
            alpha     = std::max(avg - delta, -VALUE_INFINITE);
            beta      = std::min(avg + delta, VALUE_INFINITE);

            // Adjust optimism based on root move's averageScore (~4 Elo)
            optimism[us]  = 117 * avg / (std::abs(avg) + 94);
...
void Search::Worker::clear() {
    ...
    for (size_t i = 1; i < reductions.size(); ++i)
        reductions[i] = int((20.98 + std::log(size_t(options["Threads"])) / 2) * std::log(i));
...
Value Search::Worker::search(
  Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
    ...
    {
        int bonus = std::clamp(-10 * int((ss - 1)->staticEval + ss->staticEval), -1798, 1454);
    ...
    if (eval < alpha - 476 - (309 - 131 * ((ss + 1)->cutoffCnt > 3)) * depth * depth)
    {
    ...
    if (!PvNode && (ss - 1)->currentMove != Move::null() && (ss - 1)->statScore < 14329
        && eval >= beta && ss->staticEval >= beta - 20 * depth + 374 && !excludedMove
        ...
        && beta > VALUE_TB_LOSS_IN_MAX_PLY)
    {
        ...
        Depth R = std::min(int(eval - beta) / 155, 6) + depth / 3 + 5;
    ...
    probCutBeta = beta + 173 - 64 * improving;
    if (
      !PvNode && depth > 3
...
moves_loop:  // When in check, search starts here
    ...
    while ((move = mp.next_move(moveCountPruning)) != Move::none())
    {
        ...
        if (!rootNode && pos.non_pawn_material(us) && bestValue > VALUE_TB_LOSS_IN_MAX_PLY)
        {
            ...
            if (capture || givesCheck)
            {
                ...
                {
                    Value futilityValue = ss->staticEval + 285 + 229 * lmrDepth
                ...
                int seeHist = std::clamp(captHist / 32, -172 * depth, 158 * depth);
            ...
            else
            {
                ...
                if (lmrDepth < 6 && history < -4341 * depth)
                    continue;

                history += 2 * thisThread->mainHistory[us][move.from_to()];

                lmrDepth += history / 4154;

                Value futilityValue =
                  ss->staticEval + (bestValue < ss->staticEval - 52 ? 144 : 53) + 138 * lmrDepth;
        ...
        if (ss->ply < thisThread->rootDepth * 2)
        {
            ...
            if (!rootNode && move == ttMove && !excludedMove
                ...
                && tte->depth() >= depth - 3)
            {
                Value singularBeta  = ttValue - (55 + 56 * (ss->ttPv && !PvNode)) * depth / 64;
                ...
                {
                    int doubleMargin = 301 * PvNode - 204 * !ttCapture;
                    int tripleMargin =
                      113 + 250 * PvNode - 281 * !ttCapture + 100 * (ss->ttPv || !ttCapture);
                    int quadMargin = 463 + 343 * PvNode - 271 * !ttCapture + 216 * ss->ttPv;
        ...
        ss->statScore = 2 * thisThread->mainHistory[us][move.from_to()]
                      + (*contHist[0])[movedPiece][move.to_sq()]
                      + (*contHist[1])[movedPiece][move.to_sq()] - 5025;

        // Decrease/increase reduction for moves with a good/bad history (~8 Elo)
        r -= ss->statScore / (12693 - std::min(depth, 14) * 114);
    ...
    {
        int bonus = (depth > 4) + (depth > 5) + (PvNode || cutNode) + ((ss - 1)->statScore < -14206)
                  + ((ss - 1)->moveCount > 9) + (!ss->inCheck && bestValue <= ss->staticEval - 127)
...
Value Search::Worker::qsearch(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth) {
    ...
    else
    {
        ...
        futilityBase = ss->staticEval + 274;
    }
...
Depth Search::Worker::reduction(bool i, Depth d, int mn, int delta) {
    ...
    return (reductionScale + 1189 - delta * 805 / rootDelta) / 1024 + (!i && reductionScale > 1225);
}
