...
Value futility_margin(Depth d, bool noTtCutNode, bool improving, bool oppWorsening) {
    Value futilityMult       = 120 - 43 * noTtCutNode;
    Value improvingDeduction = 53 * improving * futilityMult / 32;
    Value worseningDeduction = (308 + 49 * improving) * oppWorsening * futilityMult / 1024;
...
Value to_corrected_static_eval(Value v, const Worker& w, const Position& pos) {
    ...
    v += cv * std::abs(cv) / 9102;
    return std::clamp(v, VALUE_TB_LOSS_IN_MAX_PLY + 1, VALUE_TB_WIN_IN_MAX_PLY - 1);
}

// History and stats update bonus, based on depth
int stat_bonus(Depth d) { return std::clamp(212 * d - 323, 0, 1268); }

// History and stats update malus, based on depth
int stat_malus(Depth d) { return (d < 4 ? 568 * d - 287 : 1361); }
...
void Search::Worker::iterative_deepening() {
           ...
           && !(limits.depth && mainThread && rootDepth > limits.depth))
    {
        ...
        for (pvIdx = 0; pvIdx < multiPV && !threads.stop; ++pvIdx)
        {
            ...
            delta     = 10 + avg * avg / 11584;
            alpha     = std::max(avg - delta, -VALUE_INFINITE);
            beta      = std::min(avg + delta, VALUE_INFINITE);

            // Adjust optimism based on root move's averageScore (~4 Elo)
            optimism[us]  = 130 * avg / (std::abs(avg) + 94);
...
void Search::Worker::clear() {
    ...
    for (size_t i = 1; i < reductions.size(); ++i)
        reductions[i] = int((20.07 + std::log(size_t(options["Threads"])) / 2) * std::log(i));
}
...
Value Search::Worker::search(
  Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
    ...
    {
        int bonus = std::clamp(-14 * int((ss - 1)->staticEval + ss->staticEval), -1651, 1331);
    ...
    if (eval < alpha - 480 - (278 - 149 * ((ss + 1)->cutoffCnt > 3)) * depth * depth)
    {
    ...
    if (!PvNode && (ss - 1)->currentMove != Move::null() && (ss - 1)->statScore < 17246
        && eval >= beta && ss->staticEval >= beta - 22 * depth + 312 && !excludedMove
    ...
    probCutBeta = beta + 176 - 62 * improving;
    if (
      !PvNode && depth > 3
...
moves_loop:  // When in check, search starts here
    ...
    while ((move = mp.next_move(moveCountPruning)) != Move::none())
    {
        ...
        if (!rootNode && pos.non_pawn_material(us) && bestValue > VALUE_TB_LOSS_IN_MAX_PLY)
        {
            ...
            if (capture || givesCheck)
            {
                ...
                if (!givesCheck && lmrDepth < 7 && !ss->inCheck)
                {
                    ...
                    int   futilityEval =
                      ss->staticEval + 299 + 271 * lmrDepth + PieceValue[capturedPiece]
            ...
            else
            {
                ...
                lmrDepth += history / 5239;

                Value futilityValue =
                  ss->staticEval + (bestValue < ss->staticEval - 56 ? 130 : 58) + 126 * lmrDepth;
        ...
        if (ss->ply < thisThread->rootDepth * 2)
        {
            ...
            if (!rootNode && move == ttMove && !excludedMove
                ...
                && tte->depth() >= depth - 3)
            {
                Value singularBeta  = ttValue - (64 + 60 * (ss->ttPv && !PvNode)) * depth / 64;
                ...
                if (value < singularBeta)
                {
                    ...
                    {
                        extension = 2 + (value < singularBeta - 8 && !ttCapture);
                        depth += depth < 14;
                    }
        ...
        ss->statScore = 2 * thisThread->mainHistory[us][move.from_to()]
                      + (*contHist[0])[movedPiece][move.to_sq()]
                      + (*contHist[1])[movedPiece][move.to_sq()]
                      + (*contHist[3])[movedPiece][move.to_sq()] - 4926;

        // Decrease/increase reduction for moves with a good/bad history (~8 Elo)
        r -= ss->statScore / 13124;
        ...
        if (value > bestValue)
        {
            ...
            if (value > alpha)
            {
                ...
                else
                {
                    ...
                    if (depth > 2 && depth < 12 && beta < 13967 && value > -13807)
                        depth -= 2;
    ...
    {
        int bonus = (depth > 5) + (PvNode || cutNode) + ((ss - 1)->statScore < -14526)
                  + ((ss - 1)->moveCount > 11)
                  + (!ss->inCheck && bestValue <= ss->staticEval - 148);
        update_continuation_histories(ss - 1, pos.piece_on(prevSq), prevSq,
...
Value Search::Worker::qsearch(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth) {
    ...
    else
    {
        ...
        futilityBase = ss->staticEval + 242;
    }
...
Depth Search::Worker::reduction(bool i, Depth d, int mn, int delta) {
    ...
    return (reductionScale + 1091 - delta * 839 / rootDelta) / 1024 + (!i && reductionScale > 990);
}
...
void update_all_stats(const Position& pos,
    ...
    {
        int bestMoveBonus = bestValue > beta + 179 ? quietMoveBonus      // larger bonus
