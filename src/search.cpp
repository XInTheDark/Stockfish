...
Value futility_margin(Depth d, bool noTtCutNode, bool improving, bool oppWorsening) {
    Value futilityMult       = 124 - 33 * noTtCutNode;
...
Value to_corrected_static_eval(Value v, const Worker& w, const Position& pos) {
    ...
    v += cv * std::abs(cv) / 4878;
    return std::clamp(v, VALUE_TB_LOSS_IN_MAX_PLY + 1, VALUE_TB_WIN_IN_MAX_PLY - 1);
}

// History and stats update bonus, based on depth
int stat_bonus(Depth d) { return std::clamp(191 * d - 313, 21, 1619); }

// History and stats update malus, based on depth
int stat_malus(Depth d) { return (d < 4 ? 721 * d - 267 : 1948); }
...
void Search::Worker::iterative_deepening() {
           ...
           && !(limits.depth && mainThread && rootDepth > limits.depth))
    {
        ...
        for (pvIdx = 0; pvIdx < multiPV; ++pvIdx)
        {
            ...
            delta     = 9 + avg * avg / 10206;
            alpha     = std::max(avg - delta, -VALUE_INFINITE);
            beta      = std::min(avg + delta, VALUE_INFINITE);

            // Adjust optimism based on root move's averageScore (~4 Elo)
            optimism[us]  = 131 * avg / (std::abs(avg) + 92);
...
void Search::Worker::clear() {
    ...
    for (size_t i = 1; i < reductions.size(); ++i)
        reductions[i] = int((17.43 + std::log(size_t(options["Threads"])) / 2) * std::log(i));
...
Value Search::Worker::search(
  Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
    ...
    {
        int bonus = std::clamp(-11 * int((ss - 1)->staticEval + ss->staticEval), -1697, 1460) + 686;
    ...
    // return a fail low.
    if (eval < alpha - 487 - 273 * depth * depth)
    {
    ...
    if (!PvNode && (ss - 1)->currentMove != Move::null() && (ss - 1)->statScore < 16517
        && eval >= beta && ss->staticEval >= beta - 21 * depth + 409 && !excludedMove
    ...
    probCutBeta = beta + 201 - 53 * improving;
    if (
      !PvNode && depth > 3
...
moves_loop:  // When in check, search starts here
    ...
    while ((move = mp.next_move(moveCountPruning)) != Move::none())
    {
        ...
        if (!rootNode && pos.non_pawn_material(us) && bestValue > VALUE_TB_LOSS_IN_MAX_PLY)
        {
            ...
            moveCountPruning =
              moveCount >= futility_move_count(improving, depth)
                             - (singularBound == BOUND_UPPER && singularValue < alpha - 53);
            ...
            if (capture || givesCheck)
            {
                ...
                {
                    Value futilityValue = ss->staticEval + 281 + 259 * lmrDepth
                ...
                int seeHist = std::clamp(captHist / 32, -176 * depth, 158 * depth);
            ...
            else
            {
                ...
                if (lmrDepth < 6 && history < -4298 * depth)
                    continue;

                history += 2 * thisThread->mainHistory[us][move.from_to()];

                lmrDepth += history / 3749;

                Value futilityValue =
                  ss->staticEval + (bestValue < ss->staticEval - 52 ? 138 : 53) + 140 * lmrDepth;
        ...
        if (ss->ply < thisThread->rootDepth * 2)
        {
            ...
            {
                Value singularBeta  = ttData.value - (55 + 77 * (ss->ttPv && !PvNode)) * depth / 64;
                ...
                {
                    int doubleMargin = 261 * PvNode - 198 * !ttCapture;
                    int tripleMargin = 108 + 261 * PvNode - 282 * !ttCapture + 99 * ss->ttPv;
        ...
        ss->statScore = 2 * thisThread->mainHistory[us][move.from_to()]
                      + (*contHist[0])[movedPiece][move.to_sq()]
                      + (*contHist[1])[movedPiece][move.to_sq()] - 4639;

        // Decrease/increase reduction for moves with a good/bad history (~8 Elo)
        r -= ss->statScore / 10429;
        ...
        if (depth >= 2 && moveCount > 1 + rootNode)
        {
            ...
            if (value > alpha && d < newDepth)
            {
                ...
                const bool doDeeperSearch    = value > (bestValue + 33 + 2 * newDepth);  // (~1 Elo)
    ...
    {
        int bonus = (109 * (depth > 5) + 117 * (PvNode || cutNode) + 136 * ((ss - 1)->moveCount > 8)
                     + 68 * (!ss->inCheck && bestValue <= ss->staticEval - 112)
                     + 150 * (!(ss - 1)->inCheck && bestValue <= -(ss - 1)->staticEval - 73));

        // Proportional to "how much damage we have to undo"
        if ((ss - 1)->statScore < -8354)
            bonus += std::clamp(-(ss - 1)->statScore / 103, 0, 265);
...
Value Search::Worker::qsearch(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth) {
    ...
    else
    {
        ...
        futilityBase = ss->staticEval + 308;
    }
...
Depth Search::Worker::reduction(bool i, Depth d, int mn, int delta) const {
    ...
    return (reductionScale + 1312 - delta * 748 / rootDelta) / 1024 + (!i && reductionScale > 1309);
}
