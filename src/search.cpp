...
Value futility_margin(Depth d, bool noTtCutNode, bool improving) {
    Value futilityMult = 123 - 45 * noTtCutNode;
...
Value to_corrected_static_eval(Value v, const Worker& w, const Position& pos) {
    ...
    v += cv * std::abs(cv) / 12116;
    return std::clamp(v, VALUE_TB_LOSS_IN_MAX_PLY + 1, VALUE_TB_WIN_IN_MAX_PLY - 1);
}

// History and stats update bonus, based on depth
int stat_bonus(Depth d) { return std::min(246 * d - 358, 1136); }

// History and stats update malus, based on depth
int stat_malus(Depth d) { return std::min(523 * d - 307, 1384); }
...
void Search::Worker::iterative_deepening() {
           ...
           && !(limits.depth && mainThread && rootDepth > limits.depth))
    {
        ...
        for (pvIdx = 0; pvIdx < multiPV && !threads.stop; ++pvIdx)
        {
            ...
            delta     = 9 + avg * avg / 12732;
            alpha     = std::max(avg - delta, -VALUE_INFINITE);
            beta      = std::min(avg + delta, VALUE_INFINITE);

            // Adjust optimism based on root move's averageScore (~4 Elo)
            optimism[us]  = 137 * avg / (std::abs(avg) + 93);
...
void Search::Worker::clear() {
    ...
    for (size_t i = 1; i < reductions.size(); ++i)
        reductions[i] = int((18.58 + std::log(size_t(options["Threads"])) / 2) * std::log(i));
}
...
Value Search::Worker::search(
  Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
    ...
    {
        int bonus = std::clamp(-14 * int((ss - 1)->staticEval + ss->staticEval), -1628, 1325);
    ...
    if (eval < alpha - 449 - (307 - 146 * ((ss + 1)->cutoffCnt > 3)) * depth * depth)
    {
    ...
    if (!ss->ttPv && depth < 12
        && eval - futility_margin(depth, cutNode && !ss->ttHit, improving)
               - (ss - 1)->statScore / 315
             >= beta
        && eval >= beta && eval < VALUE_TB_WIN_IN_MAX_PLY  // smaller than TB wins
        && (!ttMove || ttCapture))
        return beta > VALUE_TB_LOSS_IN_MAX_PLY ? (eval + beta) / 2 : eval;

    // Step 9. Null move search with verification search (~35 Elo)
    if (!PvNode && (ss - 1)->currentMove != Move::null() && (ss - 1)->statScore < 15926
        && eval >= beta && eval >= ss->staticEval && ss->staticEval >= beta - 20 * depth + 329
    ...
    probCutBeta = beta + 160 - 70 * improving;
    if (
      !PvNode && depth > 3
...
moves_loop:  // When in check, search starts here
    ...
    while ((move = mp.next_move(moveCountPruning)) != Move::none())
    {
        ...
        if (!rootNode && pos.non_pawn_material(us) && bestValue > VALUE_TB_LOSS_IN_MAX_PLY)
        {
            ...
            if (capture || givesCheck)
            {
                ...
                if (!givesCheck && lmrDepth < 7 && !ss->inCheck)
                {
                    ...
                    int   futilityEval =
                      ss->staticEval + 299 + 288 * lmrDepth + PieceValue[capturedPiece]
            ...
            else
            {
                ...
                if (lmrDepth < 6 && history < -4221 * depth)
                    continue;

                history += 2 * thisThread->mainHistory[us][move.from_to()];

                lmrDepth += history / 6017;

                // Futility pruning: parent node (~13 Elo)
                if (!ss->inCheck && lmrDepth < 15
                    && ss->staticEval + (bestValue < ss->staticEval - 57 ? 146 : 55)
                           + 123 * lmrDepth
                         <= alpha)
                    continue;
        ...
        if (ss->ply < thisThread->rootDepth * 2)
        {
            ...
            if (!rootNode && move == ttMove && !excludedMove
                ...
                && tte->depth() >= depth - 3)
            {
                Value singularBeta  = ttValue - (60 + 56 * (ss->ttPv && !PvNode)) * depth / 64;
                ...
                if (value < singularBeta)
                {
                    ...
                    {
                        extension = 2 + (value < singularBeta - 81 && !ttCapture);
                        depth += depth < 15;
                    }
                }
        ...
        ss->statScore = 2 * thisThread->mainHistory[us][move.from_to()]
                      + (*contHist[0])[movedPiece][move.to_sq()]
                      + (*contHist[1])[movedPiece][move.to_sq()]
                      + (*contHist[3])[movedPiece][move.to_sq()] - 4547;

        // Decrease/increase reduction for moves with a good/bad history (~8 Elo)
        r -= ss->statScore / 13477;
        ...
        if (value > bestValue)
        {
            ...
            if (value > alpha)
            {
                ...
                else
                {
                    ...
                    if (depth > 2 && depth < 12 && beta < 13801 && value > -12024)
                        depth -= 2;
    ...
    {
        int bonus = (depth > 5) + (PvNode || cutNode) + ((ss - 1)->statScore < -15908)
                  + ((ss - 1)->moveCount > 10);
        update_continuation_histories(ss - 1, pos.piece_on(prevSq), prevSq,
...
Depth Search::Worker::reduction(bool i, Depth d, int mn, int delta) {
    ...
    return (reductionScale + 1063 - delta * 750 / rootDelta) / 1024 + (!i && reductionScale > 890);
}
