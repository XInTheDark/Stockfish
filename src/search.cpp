...
moves_loop: // When in check, search starts here
    ...
    while ((move = mp.next_move(moveCountPruning)) != MOVE_NONE)
    {
          ...
          && bestValue > VALUE_TB_LOSS_IN_MAX_PLY)
      {
              ...
              || givesCheck)
          {
              ...
              if (   !givesCheck
                  && lmrDepth < 7
                  && !ss->inCheck
                  && ss->staticEval + 178 + 227 * lmrDepth + PieceValue[pos.piece_on(to_sq(move))]
          ...
          else
          {
              ...
              lmrDepth += history / 6697;
              lmrDepth = std::max(lmrDepth, -2);

              // Futility pruning: parent node (~13 Elo)
              if (   !ss->inCheck
                  && lmrDepth < 11
                  && ss->staticEval + 115 + 137 * lmrDepth <= alpha)
                  continue;
      ...
      if (ss->ply < thisThread->rootDepth * 2)
      {
          ...
          if (   !rootNode
           ...
           /* &&  ttValue != VALUE_NONE Already implicit in the next condition */
              ...
              &&  tte->depth() >= depth - 3)
          {
              Value singularBeta = ttValue - (74 + 63 * (ss->ttPv && !PvNode)) * depth / 64;
          ...
          // Quiet ttMove extensions (~1 Elo)
                   ...
                   && move == ttMove
                   && move == ss->killers[0]
                   && (*contHist[0])[movedPiece][to_sq(move)] >= 5100)
              extension = 1;
      }
      ...
      r -= ss->statScore / (10887 + 4882 * (depth > 5 && depth < 23));
              ...
              || (cutNode && (ss-1)->moveCount > 1)))
      {
          ...
          if (value > alpha && d < newDepth)
          {
              ...
              const bool doDeeperSearch = value > (bestValue + 63 + 12 * (newDepth - d));
              const bool doEvenDeeperSearch = value > alpha + 699 && ss->doubleExtensions <= 6;
    ...
    */
    ...
    {
        int bonus = (depth > 5) + (PvNode || cutNode) + (bestValue < alpha - 735) + ((ss-1)->moveCount > 12);
  ...
  void update_all_stats(const Position& pos, Stack* ss, Move bestMove, Value bestValue, Value beta, Square prevSq,
    ...
    {
        int bestMoveBonus = bestValue > beta + 134 ? quietMoveBonus  // larger bonus
