...
Value futility_margin(Depth d, bool noTtCutNode, bool improving, bool oppWorsening) {
    Value futilityMult       = 125 - 42 * noTtCutNode;
...
Value to_corrected_static_eval(Value v, const Worker& w, const Position& pos) {
    ...
    v += cv * std::abs(cv) / 5442;
    return std::clamp(v, VALUE_TB_LOSS_IN_MAX_PLY + 1, VALUE_TB_WIN_IN_MAX_PLY - 1);
}

// History and stats update bonus, based on depth
int stat_bonus(Depth d) { return std::clamp(201 * d - 273, 17, 1770); }

// History and stats update malus, based on depth
int stat_malus(Depth d) { return (d < 4 ? 815 * d - 275 : 1800); }
...
void Search::Worker::iterative_deepening() {
           ...
           && !(limits.depth && mainThread && rootDepth > limits.depth))
    {
        ...
        for (pvIdx = 0; pvIdx < multiPV && !threads.stop; ++pvIdx)
        {
            ...
            delta     = 9 + avg * avg / 11487;
            alpha     = std::max(avg - delta, -VALUE_INFINITE);
            beta      = std::min(avg + delta, VALUE_INFINITE);

            // Adjust optimism based on root move's averageScore (~4 Elo)
            optimism[us]  = 133 * avg / (std::abs(avg) + 94);
...
void Search::Worker::clear() {
    ...
    for (size_t i = 1; i < reductions.size(); ++i)
        reductions[i] = int((19.76 + std::log(size_t(options["Threads"])) / 2) * std::log(i));
...
Value Search::Worker::search(
  Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
    ...
    {
        int bonus = std::clamp(-12 * int((ss - 1)->staticEval + ss->staticEval), -1583, 1353);
    ...
    // return a fail low.
    if (eval < alpha - 481 - 338 * depth * depth)
    {
    ...
    if (!PvNode && (ss - 1)->currentMove != Move::null() && (ss - 1)->statScore < 14061
        && eval >= beta && ss->staticEval >= beta - 20 * depth + 391 && !excludedMove
    ...
    probCutBeta = beta + 172 - 56 * improving;
    if (
      !PvNode && depth > 3
...
moves_loop:  // When in check, search starts here
    ...
    while ((move = mp.next_move(moveCountPruning)) != Move::none())
    {
        ...
        if (!rootNode && pos.non_pawn_material(us) && bestValue > VALUE_TB_LOSS_IN_MAX_PLY)
        {
            ...
            if (capture || givesCheck)
            {
                ...
                {
                    Value futilityValue = ss->staticEval + 257 + 242 * lmrDepth
                ...
                int seeHist = std::clamp(captHist * 69 / 2048, -180 * depth, 148 * depth);
            ...
            else
            {
                ...
                if (lmrDepth < 6 && history < -4436 * depth)
                    continue;

                history += 2 * thisThread->mainHistory[us][move.from_to()];

                lmrDepth += history / 3776;

                Value futilityValue =
                  ss->staticEval + (bestValue < ss->staticEval - 46 ? 156 : 56) + 129 * lmrDepth;
        ...
        if (ss->ply < thisThread->rootDepth * 2)
        {
            ...
            if (!rootNode && move == ttMove && !excludedMove
                ...
                && tte->depth() >= depth - 3)
            {
                Value singularBeta  = ttValue - (53 + 65 * (ss->ttPv && !PvNode)) * depth / 64;
                ...
                {
                    int doubleMargin = 298 * PvNode - 195 * !ttCapture;
                    int tripleMargin = 116 + 272 * PvNode - 291 * !ttCapture + 98 * ss->ttPv;
                    int quadMargin = 492 + 334 * PvNode - 281 * !ttCapture + 211 * ss->ttPv;
        ...
        ss->statScore = 2 * thisThread->mainHistory[us][move.from_to()]
                      + (*contHist[0])[movedPiece][move.to_sq()]
                      + (*contHist[1])[movedPiece][move.to_sq()] - 5732;

        // Decrease/increase reduction for moves with a good/bad history (~8 Elo)
        r -= ss->statScore / (13128 - std::min(depth, 11) * 112);
    ...
    {
        int bonus = (118 * (depth > 5) + 114 * (PvNode || cutNode)
                     + 195 * ((ss - 1)->statScore < -13425) + 118 * ((ss - 1)->moveCount > 9)
                     + 61 * (!ss->inCheck && bestValue <= ss->staticEval - 115)
                     + 135 * (!(ss - 1)->inCheck && bestValue <= -(ss - 1)->staticEval - 79));
        update_continuation_histories(ss - 1, pos.piece_on(prevSq), prevSq,
...
Depth Search::Worker::reduction(bool i, Depth d, int mn, int delta) {
    ...
    return (reductionScale + 1270 - delta * 704 / rootDelta) / 1024 + (!i && reductionScale > 1299);
}
