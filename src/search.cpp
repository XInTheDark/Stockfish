...
Value futility_margin(Depth d, bool noTtCutNode, bool improving, bool oppWorsening) {
    Value futilityMult       = 122 - 38 * noTtCutNode;
...
Value to_corrected_static_eval(Value v, const Worker& w, const Position& pos) {
    ...
    v += cv * std::abs(cv) / 4374;
    return std::clamp(v, VALUE_TB_LOSS_IN_MAX_PLY + 1, VALUE_TB_WIN_IN_MAX_PLY - 1);
}

// History and stats update bonus, based on depth
int stat_bonus(Depth d) { return std::min(182 * d - 109, 1652); }

// History and stats update malus, based on depth
int stat_malus(Depth d) { return (d < 4 ? 792 * d - 264 : 2100); }
...
void Search::Worker::clear() {
    ...
    for (size_t i = 1; i < reductions.size(); ++i)
        reductions[i] = int((20.59 + std::log(size_t(options["Threads"])) / 2) * std::log(i));
...
Value Search::Worker::search(
  Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
    ...
    {
        int bonus = std::clamp(-11 * int((ss - 1)->staticEval + ss->staticEval), -1669, 1466) + 706;
    ...
    if (eval < alpha - 487 - 309 * depth * depth)
    {
    ...
    if (!PvNode && (ss - 1)->currentMove != Move::null() && (ss - 1)->statScore < 13218
        && eval >= beta && ss->staticEval >= beta - 21 * depth + 434 && !excludedMove
...
moves_loop:  // When in check, search starts here
    ...
    while ((move = mp.next_move(moveCountPruning)) != Move::none())
    {
        ...
        if (!rootNode && pos.non_pawn_material(us) && bestValue > VALUE_TB_LOSS_IN_MAX_PLY)
        {
            ...
            if (capture || givesCheck)
            {
                ...
                {
                    Value futilityValue = ss->staticEval + 292 + 263 * lmrDepth
                ...
                int seeHist = std::clamp(captHist / 32, -176 * depth, 164 * depth);
            ...
            else
            {
                ...
                if (lmrDepth < 6 && history < -3844 * depth)
                    continue;

                history += 2 * thisThread->mainHistory[us][move.from_to()];

                lmrDepth += history / 3690;

                Value futilityValue =
                  ss->staticEval + (bestValue < ss->staticEval - 52 ? 141 : 55) + 144 * lmrDepth;
        ...
        if (ss->ply < thisThread->rootDepth * 2)
        {
            ...
            {
                Value singularBeta  = ttData.value - (55 + 72 * (ss->ttPv && !PvNode)) * depth / 64;
                ...
                {
                    int doubleMargin = 288 * PvNode - 203 * !ttCapture;
                    int tripleMargin = 98 + 271 * PvNode - 267 * !ttCapture + 91 * ss->ttPv;
        ...
        ss->statScore = 2 * thisThread->mainHistory[us][move.from_to()]
                      + (*contHist[0])[movedPiece][move.to_sq()]
                      + (*contHist[1])[movedPiece][move.to_sq()] - 4216;

        // Decrease/increase reduction for moves with a good/bad history (~8 Elo)
        r -= ss->statScore / 10095;
    ...
    {
        int bonus = (116 * (depth > 5) + 114 * (PvNode || cutNode) + 126 * ((ss - 1)->moveCount > 8)
                     + 68 * (!ss->inCheck && bestValue <= ss->staticEval - 104)
                     + 150 * (!(ss - 1)->inCheck && bestValue <= -(ss - 1)->staticEval - 81));

        // Proportional to "how much damage we have to undo"
        bonus += std::clamp(-(ss - 1)->statScore / 102, -44, 293);

        update_continuation_histories(ss - 1, pos.piece_on(prevSq), prevSq,
                                      stat_bonus(depth) * bonus / 113);
        thisThread->mainHistory[~us][((ss - 1)->currentMove).from_to()]
          << stat_bonus(depth) * bonus / 198;
...
Value Search::Worker::qsearch(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth) {
    ...
    else
    {
        ...
        futilityBase = ss->staticEval + 317;
    }
...
Depth Search::Worker::reduction(bool i, Depth d, int mn, int delta) const {
    ...
    return (reductionScale + 1243 - delta * 760 / rootDelta) / 1024 + (!i && reductionScale > 1335);
}
