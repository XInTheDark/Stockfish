...
Value futility_margin(Depth d, bool noTtCutNode, bool improving) {
    return Value((109 - 41 * noTtCutNode) * (d - improving));
}
...
Depth reduction(bool i, Depth d, int mn, Value delta, Value rootDelta) {
    ...
    return (reductionScale + 1297 - int(delta) * 890 / int(rootDelta)) / 1024
         + (!i && reductionScale > 856);
}
...
int stat_bonus(Depth d) { return std::min(267 * d - 358, 1171); }

// History and stats update malus, based on depth
int stat_malus(Depth d) { return std::min(417 * d - 352, 1200); }
...
void Thread::search() {
           ...
           && !(Limits.depth && mainThread && rootDepth > Limits.depth))
    {
        ...
        for (pvIdx = 0; pvIdx < multiPV && !Threads.stop; ++pvIdx)
        {
            ...
            delta     = Value(9) + int(avg) * avg / 14331;
            alpha     = std::max(avg - delta, -VALUE_INFINITE);
            beta      = std::min(avg + delta, VALUE_INFINITE);

            // Adjust optimism based on root move's averageScore (~4 Elo)
            optimism[us]  = 122 * avg / (std::abs(avg) + 106);
...
Value search(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
    ...
    {
        int bonus = std::clamp(-13 * int((ss - 1)->staticEval + ss->staticEval), -1548, 1509);
    ...
    if (eval < alpha - 471 - (278 - 171 * ((ss + 1)->cutoffCnt > 3)) * depth * depth)
    {
    ...
    if (!ss->ttPv && depth < 9
        && eval - futility_margin(depth, cutNode && !ss->ttHit, improving)
               - (ss - 1)->statScore / 344
             >= beta
        && eval >= beta && eval < 30062  // smaller than TB wins
        && (!ttMove || ttCapture))
        return (eval + beta) / 2;

    // Step 9. Null move search with verification search (~35 Elo)
    if (!PvNode && (ss - 1)->currentMove != MOVE_NULL && (ss - 1)->statScore < 18032 && eval >= beta
        && eval >= ss->staticEval && ss->staticEval >= beta - 23 * depth + 295 && !excludedMove
    ...
    while ((move = mp.next_move(moveCountPruning)) != MOVE_NONE)
    {
        ...
        if (!rootNode && pos.non_pawn_material(us) && bestValue > VALUE_TB_LOSS_IN_MAX_PLY)
        {
            ...
            if (capture || givesCheck)
            {
                ...
                if (!givesCheck && lmrDepth < 7 && !ss->inCheck)
                {
                    ...
                    int   futilityEval =
                      ss->staticEval + 241 + 302 * lmrDepth + PieceValue[capturedPiece]
            ...
            else
            {
                ...
                lmrDepth += history / 7717;
                lmrDepth = std::max(lmrDepth, -1);

                // Futility pruning: parent node (~13 Elo)
                if (!ss->inCheck && lmrDepth < 14
                    && ss->staticEval + (bestValue < ss->staticEval - 57 ? 126 : 70)
                           + 120 * lmrDepth
                         <= alpha)
                    continue;
        ...
        if (ss->ply < thisThread->rootDepth * 2)
        {
            ...
            if (!rootNode && move == ttMove && !excludedMove
                ...
                && tte->depth() >= depth - 3)
            {
                Value singularBeta  = ttValue - (71 + 56 * (ss->ttPv && !PvNode)) * depth / 64;
                ...
                if (value < singularBeta)
                {
                    ...
                    if (!PvNode && value < singularBeta - 15 && ss->doubleExtensions <= 11)
                    {
                        extension = 2;
                        depth += depth < 15;
                    }
            ...
            // Quiet ttMove extensions (~1 Elo)
                     ...
                     && (*contHist[0])[movedPiece][to_sq(move)] >= 4063)
                extension = 1;
        ...
        ss->statScore = 2 * thisThread->mainHistory[us][from_to(move)]
                      ...
                      + (*contHist[1])[movedPiece][to_sq(move)]
                      + (*contHist[3])[movedPiece][to_sq(move)] - 3661;

        // Decrease/increase reduction for moves with a good/bad history (~25 Elo)
        r -= ss->statScore / 14866;
        ...
        if (value > bestValue)
        {
            ...
            if (value > alpha)
            {
                ...
                else
                {
                    ...
                    if (depth > 2 && depth < 11 && beta < 14117 && value > -12090)
                        depth -= 2;
...
Value qsearch(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth) {
    ...
    else
    {
        ...
        futilityBase = ss->staticEval + 183;
    }
...
void update_all_stats(const Position& pos,
    ...
    if (!pos.capture_stage(bestMove))
    {
        ...
        int moveMalus = bestValue > beta + 170 ? quietMoveMalus      // larger malus
