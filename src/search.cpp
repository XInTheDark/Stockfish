...
Depth reduction(bool improving, Depth d, int moveCount, Value delta, Value rootDelta, Value optimism) {
    int reductionScale = Reductions[d] * Reductions[moveCount];
    return (reductionScale + 1487 - int(delta) * 976 / int(rootDelta) - 6 * optimism) / 1024
         + (!improving && reductionScale > 808);
}
...
moves_loop:  // When in check, search starts here
    ...
    while ((move = mp.next_move(moveCountPruning)) != MOVE_NONE)
    {
        ...
        Depth r = reduction(improving, depth, moveCount, delta, thisThread->rootDelta, thisThread->optimism[us]);
