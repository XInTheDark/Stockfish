...
Value futility_margin(Depth d, bool noTtCutNode, bool improving) {
    return Value((121 - 38 * noTtCutNode) * (d - improving));
}
...
Depth reduction(bool i, Depth d, int mn, Value delta, Value rootDelta) {
    ...
    return (reductionScale + 1356 - int(delta) * 940 / int(rootDelta)) / 1024
         + (!i && reductionScale > 851);
}
...
int stat_bonus(Depth d) { return std::min(332 * d - 477, 1609); }

// History and stats update malus, based on depth
int stat_malus(Depth d) { return std::min(497 * d - 428, 1291); }
...
void Search::init() {
    ...
    for (int i = 1; i < MAX_MOVES; ++i)
        Reductions[i] = int((18.14 + std::log(Threads.size()) / 2) * std::log(i));
}
...
void Thread::search() {
           ...
           && !(Limits.depth && mainThread && rootDepth > Limits.depth))
    {
        ...
        for (pvIdx = 0; pvIdx < multiPV && !Threads.stop; ++pvIdx)
        {
            ...
            delta     = Value(10) + int(avg) * avg / 14649;
            alpha     = std::max(avg - delta, -VALUE_INFINITE);
            beta      = std::min(avg + delta, VALUE_INFINITE);

            // Adjust optimism based on root move's averageScore (~4 Elo)
            optimism[us]  = 106 * (avg + 36) / (std::abs(avg + 28) + 118);
            optimism[~us] = -107 * (avg + 41) / (std::abs(avg + 14) + 121);
...
Value search(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
    ...
    {
        int bonus = std::clamp(-18 * int((ss - 1)->staticEval + ss->staticEval), -1871, 1871);
    ...
    if (eval < alpha - 447 - (257 - 166 * ((ss + 1)->cutoffCnt > 3)) * depth * depth)
    {
    ...
    if (!ss->ttPv && depth < 8
        && eval - futility_margin(depth, cutNode && !ss->ttHit, improving)
               - (ss - 1)->statScore / 350
             >= beta
        && eval >= beta && eval < 27024  // smaller than TB wins
        && (!ttMove || ttCapture))
        return eval;

    // Step 9. Null move search with verification search (~35 Elo)
    if (!PvNode && (ss - 1)->currentMove != MOVE_NULL && (ss - 1)->statScore < 18356 && eval >= beta
        && eval >= ss->staticEval && ss->staticEval >= beta - 23 * depth + 276 && !excludedMove
        ...
        && beta > VALUE_TB_LOSS_IN_MAX_PLY)
    {
        ...
        Depth R = std::min(int(eval - beta) / 146, 6) + depth / 3 + 4;
    ...
    probCutBeta = beta + 170 - 78 * improving;
...
moves_loop:  // When in check, search starts here
    ...
    while ((move = mp.next_move(moveCountPruning)) != MOVE_NONE)
    {
        ...
        if (!rootNode && pos.non_pawn_material(us) && bestValue > VALUE_TB_LOSS_IN_MAX_PLY)
        {
            ...
            if (capture || givesCheck)
            {
                ...
                if (!givesCheck && lmrDepth < 6 && !ss->inCheck)
                {
                    ...
                    int   futilityEval =
                      ss->staticEval + 224 + 279 * lmrDepth + PieceValue[capturedPiece]
                      + captureHistory[movedPiece][to_sq(move)][type_of(capturedPiece)] * 3 / 20;
                    if (futilityEval < alpha)
                        continue;
                }

                // SEE based pruning for captures and checks (~11 Elo)
                if (!pos.see_ge(move, Value(-186) * depth))
                    continue;
            }
            else
            {
                ...
                if (lmrDepth < 6 && history < -4087 * depth)
                    continue;

                history += 2 * thisThread->mainHistory[us][from_to(move)];

                lmrDepth += history / 7258;
                lmrDepth = std::max(lmrDepth, -1);

                // Futility pruning: parent node (~13 Elo)
                if (!ss->inCheck && lmrDepth < 13
                    && ss->staticEval + (bestValue < ss->staticEval - 63 ? 127 : 80)
                           + 124 * lmrDepth
                         <= alpha)
                    continue;
        ...
        if (ss->ply < thisThread->rootDepth * 2)
        {
            ...
            if (!rootNode && move == ttMove && !excludedMove
                ...
                && tte->depth() >= depth - 3)
            {
                Value singularBeta  = ttValue - (61 + 56 * (ss->ttPv && !PvNode)) * depth / 64;
                ...
                if (value < singularBeta)
                {
                    ...
                    if (!PvNode && value < singularBeta - 18 && ss->doubleExtensions <= 12)
                    {
                        extension = 2;
                        depth += depth < 16;
                    }
            ...
            // Quiet ttMove extensions (~1 Elo)
                     ...
                     && (*contHist[0])[movedPiece][to_sq(move)] >= 3976)
                extension = 1;
        }
        ...
        ss->statScore = 2 * thisThread->mainHistory[us][from_to(move)]
                      ...
                      + (*contHist[1])[movedPiece][to_sq(move)]
                      + (*contHist[3])[movedPiece][to_sq(move)] - 3620;

        // Decrease/increase reduction for moves with a good/bad history (~25 Elo)
        r -= ss->statScore / (11745 + 3500 * (depth > 5 && depth < 22));
            ...
            && (!ss->ttPv || !capture || (cutNode && (ss - 1)->moveCount > 1)))
        {
            ...
            if (value > alpha && d < newDepth)
            {
                ...
                const bool doDeeperSearch     = value > (bestValue + 50 + 11 * (newDepth - d));
                const bool doEvenDeeperSearch = value > alpha + 657 && ss->doubleExtensions <= 6;
        ...
        if (value > bestValue)
        {
            ...
            if (value > alpha)
            {
                ...
                else
                {
                    ...
                    if (depth > 2 && depth < 13 && beta < 13741 && value > -12003)
                        depth -= 2;
    ...
    {
        int bonus = (depth > 6) + (PvNode || cutNode) + (bestValue < alpha - 598)
                  + ((ss - 1)->moveCount > 10);
        update_continuation_histories(ss - 1, pos.piece_on(prevSq), prevSq,
...
void update_all_stats(const Position& pos,
    ...
    {
        int bestMoveBonus = bestValue > beta + 162 ? quietMoveBonus      // larger bonus
        ...
        int moveMalus = bestValue > beta + 160 ? quietMoveMalus      // larger malus
