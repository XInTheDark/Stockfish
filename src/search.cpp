...
void Search::Worker::iterative_deepening() {
           ...
           && !(limits.depth && mainThread && rootDepth > limits.depth))
    {
        ...
        if (limits.use_time_management() && !threads.stop && !mainThread->stopOnPonderhit)
        {
            ...
            double fallingEval = (1080 + 220 * (mainThread->bestPreviousAverageScore - bestValue)
                                  + 99 * (mainThread->iterValue[iterIdx] - bestValue))
                               / 10000.0;
            fallingEval = std::clamp(fallingEval, 0.619, 1.671);

            // If the bestMove is stable over several iterations, reduce time accordingly
            timeReduction    = lastBestMoveDepth + 9 < completedDepth ? 1.525 : 0.702;
            double reduction = (1.49 + mainThread->previousTimeReduction) / (2.22 * timeReduction);
            double bestMoveInstability = 1 + 1.90 * totBestMoveChanges / threads.size();
            double recapture           = limits.capSq == rootMoves[0].pv[0].to_sq() ? 0.917 : 0.977;
            ...
            if (completedDepth >= 10 && nodesEffort >= 93 && elapsedTime > totalTime * 0.734
                && !mainThread->ponder)
                threads.stop = true;
