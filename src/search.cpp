...
Value futility_margin(Depth d, bool noTtCutNode, bool improving) {
    return Value((119 - 46 * noTtCutNode) * (d - improving));
}
...
Depth reduction(bool i, Depth d, int mn, Value delta, Value rootDelta) {
    ...
    return (reductionScale + 1633 - int(delta) * 1015 / int(rootDelta)) / 1024
         + (!i && reductionScale > 912);
}
...
int stat_bonus(Depth d) { return std::min(339 * d - 433, 1515); }

// History and stats update malus, based on depth
int stat_malus(Depth d) { return std::min(502 * d - 447, 1476); }
...
void Search::init() {
    ...
    for (int i = 1; i < MAX_MOVES; ++i)
        Reductions[i] = int((19.86 + std::log(Threads.size()) / 2) * std::log(i));
}
...
void Thread::search() {
           ...
           && !(Limits.depth && mainThread && rootDepth > Limits.depth))
    {
        ...
        for (pvIdx = 0; pvIdx < multiPV && !Threads.stop; ++pvIdx)
        {
            ...
            delta     = Value(10) + int(avg) * avg / 15301;
            alpha     = std::max(avg - delta, -VALUE_INFINITE);
            beta      = std::min(avg + delta, VALUE_INFINITE);

            // Adjust optimism based on root move's averageScore (~4 Elo)
            optimism[us]  = 106 * (avg + 36) / (std::abs(avg + 32) + 122);
            optimism[~us] = -115 * (avg + 42) / (std::abs(avg + 15) + 127);
...
Value search(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
    ...
    {
        int bonus = std::clamp(-18 * int((ss - 1)->staticEval + ss->staticEval), -1897, 1897);
    ...
    if (eval < alpha - 515 - (276 - 173 * ((ss + 1)->cutoffCnt > 3)) * depth * depth)
    {
    ...
    if (!ss->ttPv && depth < 10
        && eval - futility_margin(depth, cutNode && !ss->ttHit, improving)
               - (ss - 1)->statScore / 319
             >= beta
        && eval >= beta && eval < 26623  // smaller than TB wins
        && (!ttMove || ttCapture))
        return eval;

    // Step 9. Null move search with verification search (~35 Elo)
    if (!PvNode && (ss - 1)->currentMove != MOVE_NULL && (ss - 1)->statScore < 20029 && eval >= beta
        && eval >= ss->staticEval && ss->staticEval >= beta - 25 * depth + 297 && !excludedMove
    ...
    probCutBeta = beta + 151 - 63 * improving;
...
moves_loop:  // When in check, search starts here
    ...
    while ((move = mp.next_move(moveCountPruning)) != MOVE_NONE)
    {
        ...
        if (!rootNode && pos.non_pawn_material(us) && bestValue > VALUE_TB_LOSS_IN_MAX_PLY)
        {
            ...
            if (capture || givesCheck)
            {
                ...
                if (!givesCheck && lmrDepth < 6 && !ss->inCheck)
                {
                    ...
                    int   futilityEval =
                      ss->staticEval + 235 + 256 * lmrDepth + PieceValue[capturedPiece]
            ...
            else
            {
                ...
                lmrDepth += history / 8387;
                lmrDepth = std::max(lmrDepth, -1);

                // Futility pruning: parent node (~13 Elo)
                if (!ss->inCheck && lmrDepth < 13
                    && ss->staticEval + (bestValue < ss->staticEval - 58 ? 137 : 71)
                           + 122 * lmrDepth
                         <= alpha)
                    continue;
        ...
        if (ss->ply < thisThread->rootDepth * 2)
        {
            ...
            if (!rootNode && move == ttMove && !excludedMove
                ...
                && tte->depth() >= depth - 3)
            {
                Value singularBeta  = ttValue - (63 + 55 * (ss->ttPv && !PvNode)) * depth / 64;
            ...
            // Quiet ttMove extensions (~1 Elo)
                     ...
                     && (*contHist[0])[movedPiece][to_sq(move)] >= 4170)
                extension = 1;
        }
        ...
        ss->statScore = 2 * thisThread->mainHistory[us][from_to(move)]
                      ...
                      + (*contHist[1])[movedPiece][to_sq(move)]
                      + (*contHist[3])[movedPiece][to_sq(move)] - 4080;

        // Decrease/increase reduction for moves with a good/bad history (~25 Elo)
        r -= ss->statScore / (10356 + 4118 * (depth > 5 && depth < 26));
            ...
            && (!ss->ttPv || !capture || (cutNode && (ss - 1)->moveCount > 1)))
        {
            ...
            if (value > alpha && d < newDepth)
            {
                ...
                const bool doEvenDeeperSearch = value > alpha + 642 && ss->doubleExtensions <= 6;
        ...
        if (value > bestValue)
        {
            ...
            if (value > alpha)
            {
                ...
                else
                {
                    ...
                    if (depth > 2 && depth < 11 && beta < 14682 && value > -11485)
                        depth -= 2;
    ...
    {
        int bonus = (depth > 6) + (PvNode || cutNode) + (bestValue < alpha - 697)
                  + ((ss - 1)->moveCount > 10);
        update_continuation_histories(ss - 1, pos.piece_on(prevSq), prevSq,
...
Value qsearch(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth) {
    ...
    else
    {
        ...
        futilityBase = ss->staticEval + 190;
    }
...
void update_all_stats(const Position& pos,
    ...
    {
        int bestMoveBonus = bestValue > beta + 158 ? quietMoveBonus      // larger bonus
        ...
        int moveMalus = bestValue > beta + 178 ? quietMoveMalus      // larger malus
