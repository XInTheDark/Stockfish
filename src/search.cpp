...
namespace {
  ...
  Depth reduction(bool i, Depth d, int mn, Value delta, Value rootDelta) {
    ...
    return (r + 1461 - int(delta) * 1233 / int(rootDelta)) / 1024 + (!i && r > 925);
  }
...
void Search::init() {
  ...
  for (int i = 1; i < MAX_MOVES; ++i)
      Reductions[i] = int((20.10 + std::log(Threads.size()) / 2) * std::log(i));
}
...
void Thread::search() {
         ...
         && !(Limits.depth && mainThread && rootDepth > Limits.depth))
  {
      ...
      for (pvIdx = 0; pvIdx < multiPV && !Threads.stop; ++pvIdx)
      {
          ...
          delta = Value(9) + int(prev) * prev / 16820;
          alpha = std::max(prev - delta,-VALUE_INFINITE);
          beta  = std::min(prev + delta, VALUE_INFINITE);

          // Adjust optimism based on root move's previousScore
          int opt = 106 * prev / (std::abs(prev) + 153);
...
namespace {
  ...
  Value search(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
    ...
    {
        int bonus = std::clamp(-19 * int((ss-1)->staticEval + ss->staticEval), -1881, 1881);
    ...
    // return a fail low.
    if (eval < alpha - 477 - 263 * depth * depth)
    {
    ...
    if (   !ss->ttPv
        &&  depth < 9
        &&  eval - futility_margin(depth, improving) - (ss-1)->statScore / 346 >= beta
        &&  eval >= beta
        &&  eval < 25911) // larger than VALUE_KNOWN_WIN, but smaller than TB wins
        return eval;

    // Step 9. Null move search with verification search (~35 Elo)
    if (   !PvNode
        && (ss-1)->currentMove != MOVE_NULL
        && (ss-1)->statScore < 17240
        &&  eval >= beta
        &&  eval >= ss->staticEval
        &&  ss->staticEval >= beta - 24 * depth + 264
        && !excludedMove
        ...
        &&  beta > VALUE_TB_LOSS_IN_MAX_PLY)
    {
        ...
        Depth R = std::min(int(eval - beta) / 176, 6) + depth / 3 + 4;
    ...
    probCutBeta = beta + 175 - 54 * improving;
...
moves_loop: // When in check, search starts here
    ...
    while ((move = mp.next_move(moveCountPruning)) != MOVE_NONE)
    {
          ...
          && bestValue > VALUE_TB_LOSS_IN_MAX_PLY)
      {
              ...
              || givesCheck)
          {
              ...
              if (   !givesCheck
                  && lmrDepth < 8
                  && !ss->inCheck
                  && ss->staticEval + 193 + 269 * lmrDepth + PieceValue[EG][pos.piece_on(to_sq(move))]
          ...
          else
          {
              ...
              lmrDepth += history / 7096;
              lmrDepth = std::max(lmrDepth, -2);

              // Futility pruning: parent node (~13 Elo)
              if (   !ss->inCheck
                  && lmrDepth < 14
                  && ss->staticEval + 113 + 142 * lmrDepth <= alpha)
                  continue;
      ...
      if (ss->ply < thisThread->rootDepth * 2)
      {
          ...
          if (   !rootNode
           ...
           /* &&  ttValue != VALUE_NONE Already implicit in the next condition */
              ...
              &&  tte->depth() >= depth - 3)
          {
              Value singularBeta = ttValue - (72 + 64 * (ss->ttPv && !PvNode)) * depth / 64;
          ...
          // Quiet ttMove extensions (~1 Elo)
                   ...
                   && move == ttMove
                   && move == ss->killers[0]
                   && (*contHist[0])[movedPiece][to_sq(move)] >= 5005)
              extension = 1;
      }
      ...
      r -= ss->statScore / (10670 + 4209 * (depth > 6 && depth < 20));
              ...
              || (cutNode && (ss-1)->moveCount > 1)))
      {
          ...
          if (value > alpha && d < newDepth)
          {
              ...
              const bool doEvenDeeperSearch = value > alpha + 659 && ss->doubleExtensions <= 7;
    ...
    */
    ...
    {
        int bonus = (depth > 5) + (PvNode || cutNode) + (bestValue < alpha - 115 * depth) + ((ss-1)->moveCount > 11);
  ...
  Value qsearch(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth) {
    ...
    while ((move = mp.next_move()) != MOVE_NONE)
    {
        ...
        if (bestValue > VALUE_TB_LOSS_IN_MAX_PLY)
        {
            ...
            if (   !capture
                && (*contHist[0])[pos.moved_piece(move)][to_sq(move)] < 752
                && (*contHist[1])[pos.moved_piece(move)][to_sq(move)] < -132)
                continue;

            // Do not search moves with bad enough SEE values (~5 Elo)
            if (!pos.see_ge(move, Value(-85)))
                continue;
        }
